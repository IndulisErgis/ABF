create FUNCTION [dbo].[ALP_ufxGetAuditFieldNewValue_MAH] 
( 
   @xml as XML, 
...
SELECT e.props.value('./FieldName[1]', 'VARCHAR(max)') as [Name] 
   , e.props.value('./NewValue[1]', 'VARCHAR(max)') as [Value] 
...
WHERE (e.props.value('./FieldName[1]', 'varchar(max)') = @FieldName) --AND (e.props.Value('NewValue') = 1) 
