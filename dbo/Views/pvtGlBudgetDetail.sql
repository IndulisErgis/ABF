...
CREATE VIEW dbo.pvtGlBudgetDetail
AS
SELECT case when h.BalType = 1 then 'Debit' when h.BalType = 0 then 'Memo' else 'Credit' end as [BalType$]
	, h.[Desc], h.AcctIdMasked, dtl.GlYear AS [Year], dtl.GlPeriod AS [Period], ISNULL(SUM(dtl.Amount), 0) AS Budget
...
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'pvtGlBudgetDetail';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'pvtGlBudgetDetail';
