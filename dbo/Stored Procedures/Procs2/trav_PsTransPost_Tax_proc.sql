...
CREATE PROCEDURE dbo.trav_PsTransPost_Tax_proc
AS
SET NOCOUNT ON
BEGIN TRY
	...
	SELECT x.TaxLocID, x.TaxClass, @PostRun, 'PS', t.TransID, NULL, NULL, h.TransDate, @FiscalPeriod, @FiscalYear, 
		SUM(SIGN(h.TransType) * x.Taxable), SUM(SIGN(h.TransType) * x.NonTaxable), SUM(SIGN(h.TransType) * x.TaxAmt), 
		0, 0, SUM(SIGN(h.TransType) * x.TaxAmt), 0, 0
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PsTransPost_Tax_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PsTransPost_Tax_proc';
