...
CREATE PROCEDURE dbo.trav_SvWorkOrderWorksheet_proc
...
--	  , d.RequestedTechID, d.HoldYN, d.EntryDate, d.[Counter], d.LocID 
...
--	 WHERE o.CustID IS NOT NULL AND d.[Status] = 0 AND d.CancelledYN = 0
--) tmp --{0}
	...
	BEGIN
		SELECT d.ID AS DispatchID
			, WorkOrderNo, w.CustID, w.SiteID, Address1, Address2
			, City, Region, Country, PostalCode, w.Attention AS Contact, w.Phone1 AS Phone
			, DispatchNo, d.EquipmentID
			, e.EquipmentNo, EquipmentDescription, e.ItemID, e.SerialNumber, e.TagNumber
	...
	ELSE
	BEGIN
		SELECT d.ID AS DispatchID, WorkOrderNo, DispatchNo, w.CustID + ' - ' + c.CustName AS CustIDName
		, w.SiteID, w.Phone1 AS Phone, w.Country, w.Attention AS Contact
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_SvWorkOrderWorksheet_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_SvWorkOrderWorksheet_proc';
