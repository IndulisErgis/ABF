...
CREATE PROCEDURE [dbo].[trav_InPhysicalCount_PrepareBatch_proc_MAHTEST] 
@BatchId pBatchId
AS
DECLARE @WmYn bit
BEGIN TRY
...
BEGIN
	...
	WHERE (@ItemIdFrom IS NULL OR i.ItemId >= @ItemIdFrom) AND (@ItemIdThru IS NULL OR i.ItemId <= @ItemIdThru)
		...
		AND (i.LottedYn = 0 OR (i.LottedYn = 1 AND t.LotNum IS NOT NULL)) 
...
BEGIN
	...
	--Serial numbers
	INSERT INTO dbo.tblInPhysCountDetail(SeqNum, SerNum, CountedUom)
	SELECT c.SeqNum,s.SerNum,c.CountedUom
