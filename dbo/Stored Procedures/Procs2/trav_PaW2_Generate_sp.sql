...
CREATE PROCEDURE [dbo].[trav_PaW2_Generate_sp]
@PaYear smallint
AS
...
(Select DCB.employeeid, sum(DCB.BOX10) DCB
	...
	FROM tblPaEmpHistDeduct H INNER JOIN tblPaDeductCode EC 
		ON H.DeductionCode = EC.DeductionCode
	WHERE EC.W2Box='10' AND H.PaYear = @PaYear
	GROUP BY  H.EmployeeID
	) DCB
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.10203.1229', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PaW2_Generate_sp';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 13344', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PaW2_Generate_sp';
