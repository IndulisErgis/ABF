...
CREATE PROCEDURE dbo.trav_PsTransPost_BuildLog_proc
AS
BEGIN TRY
	...
	SELECT h.TransDate, CASE d.LineType WHEN 3 THEN 30 WHEN 4 THEN 40 END, CASE WHEN ISNULL(d.Descr,'') = '' THEN 
		...
		CASE d.LineType WHEN 3 THEN c.GLAcctFreight WHEN 4 THEN c.GLAcctMisc END, -1 * SIGN(TransType) * d.ExtPrice, 
		CASE WHEN SIGN(TransType) * d.ExtPrice > 0 THEN ABS(d.ExtPrice) ELSE 0 END, 
		CASE WHEN SIGN(TransType) * d.ExtPrice > 0 THEN 0  ELSE ABS(d.ExtPrice) END, h.ID, @CurrBase, 1, 
		CASE WHEN SIGN(TransType) * d.ExtPrice > 0 THEN ABS(d.ExtPrice) ELSE 0 END, 
		CASE WHEN SIGN(TransType) * d.ExtPrice > 0 THEN 0  ELSE ABS(d.ExtPrice) END
	...
	SELECT h.TransDate, CASE d.LineType WHEN -2 THEN 50 WHEN -3 THEN 60 WHEN -4 THEN 70 END, CASE WHEN ISNULL(d.Descr,'') = '' THEN 
		...
		SIGN(TransType) * d.ExtPrice, CASE WHEN SIGN(TransType) * d.ExtPrice > 0 THEN 0  ELSE ABS(d.ExtPrice) END, 
		CASE WHEN SIGN(TransType) * d.ExtPrice > 0 THEN ABS(d.ExtPrice) ELSE 0 END, h.ID, @CurrBase, 1, 
		CASE WHEN SIGN(TransType) * d.ExtPrice > 0 THEN 0  ELSE ABS(d.ExtPrice) END, 
		CASE WHEN SIGN(TransType) * d.ExtPrice > 0 THEN ABS(d.ExtPrice) ELSE 0 END
	FROM #PsTransList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
		INNER JOIN dbo.tblPsTransDetail d ON h.ID = d.HeaderID 
	...
	INSERT INTO #TransPostLog (Transdate, [Grouping], Descr, Reference, DistCode, GlAcct, AmountFgn, 
		CreditAmount, DebitAmount, LinkID, CurrencyId, ExchRate, CreditAmtFgn, DebitAmtFgn)   
	SELECT h.TransDate, 10, SUBSTRING(ISNULL(d.Descr,''), 1, 30), h.BillToID, t.DistCode, g.GLAcctCogs, SIGN(TransType) * i.ExtCost, 
		CASE WHEN SIGN(TransType) * i.ExtCost > 0 THEN 0  ELSE ABS(i.ExtCost) END, 
		CASE WHEN SIGN(TransType) * i.ExtCost > 0 THEN ABS(i.ExtCost) ELSE 0 END, h.ID, @CurrBase, 1, 
		CASE WHEN SIGN(TransType) * i.ExtCost > 0 THEN 0  ELSE ABS(i.ExtCost) END, 
		CASE WHEN SIGN(TransType) * i.ExtCost > 0 THEN ABS(i.ExtCost) ELSE 0 END
	FROM #PsTransList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
		INNER JOIN dbo.tblPsTransDetail d ON h.ID = d.HeaderID 
	...
	INSERT INTO #TransPostLog (Transdate, [Grouping], Descr, Reference, DistCode, GlAcct, AmountFgn, 
		CreditAmount, DebitAmount, LinkID, CurrencyId, ExchRate, CreditAmtFgn, DebitAmtFgn)   
	SELECT h.TransDate, 20, SUBSTRING(ISNULL(d.Descr,''), 1, 30), h.BillToID, t.DistCode, g.GLAcctInv, -SIGN(TransType) * i.ExtCost, 
		CASE WHEN SIGN(TransType) * i.ExtCost > 0 THEN ABS(i.ExtCost) ELSE 0 END, 
		CASE WHEN SIGN(TransType) * i.ExtCost > 0 THEN 0 ELSE ABS(i.ExtCost) END, h.ID, @CurrBase, 1, 
		CASE WHEN SIGN(TransType) * i.ExtCost > 0 THEN ABS(i.ExtCost) ELSE 0 END, 
		CASE WHEN SIGN(TransType) * i.ExtCost > 0 THEN 0 ELSE ABS(i.ExtCost) END
	FROM #PsTransList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
		INNER JOIN dbo.tblPsTransDetail d ON h.ID = d.HeaderID 
	...
	SELECT h.TransDate, 80, SUBSTRING(ISNULL(d.Descr,''), 1, 30), h.BillToID, t.DistCode, 
		CASE WHEN i.DetailID IS NULL THEN c.GLAcctSales ELSE g.GLAcctSales END, -1 * SIGN(TransType) * d.ExtPrice, 
		CASE WHEN SIGN(TransType) * d.ExtPrice > 0 THEN ABS(d.ExtPrice) ELSE 0 END, 
		CASE WHEN SIGN(TransType) * d.ExtPrice > 0 THEN 0  ELSE ABS(d.ExtPrice) END, h.ID, @CurrBase, 1, 
		CASE WHEN SIGN(TransType) * d.ExtPrice > 0 THEN ABS(d.ExtPrice) ELSE 0 END, 
		CASE WHEN SIGN(TransType) * d.ExtPrice > 0 THEN 0  ELSE ABS(d.ExtPrice) END
	...
	SELECT h.TransDate, 90, SUBSTRING('Sales Tax' + ' / ' + x.TaxLocID, 1, 30), h.BillToID, t.DistCode, l.GLAcct, 
		-1 * SIGN(TransType) * x.TaxAmt, CASE WHEN SIGN(TransType) * x.TaxAmt > 0 THEN ABS(x.TaxAmt) ELSE 0 END, 
		CASE WHEN SIGN(TransType) * x.TaxAmt > 0 THEN 0  ELSE ABS(x.TaxAmt) END, h.ID, @CurrBase, 1, 
		CASE WHEN SIGN(TransType) * x.TaxAmt > 0 THEN ABS(x.TaxAmt) ELSE 0 END, 
		CASE WHEN SIGN(TransType) * x.TaxAmt > 0 THEN 0  ELSE ABS(x.TaxAmt) END
	FROM #PsTransList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
	...
	--Transaction payment cash account
	INSERT INTO #TransPostLog (Transdate, [Grouping], Descr, Reference, DistCode, GlAcct, AmountFgn, 
		CreditAmount, DebitAmount, LinkID, CurrencyId, ExchRate, CreditAmtFgn, DebitAmtFgn)   
	...
	INSERT INTO #TransPostLog (Transdate, [Grouping], Descr, Reference, DistCode, GlAcct, AmountFgn, 
		CreditAmount, DebitAmount, LinkID, CurrencyId, ExchRate, CreditAmtFgn, DebitAmtFgn)   
	SELECT h.TransDate, 110, t.InvoiceNum + '/' + 'A/R', h.BillToID, t.DistCode, c.GLAcctReceivables, SIGN(TransType) * t.NetDue, 
		CASE WHEN SIGN(h.TransType) * t.NetDue > 0 THEN 0  ELSE ABS(t.NetDue) END, 
		CASE WHEN SIGN(h.TransType) * t.NetDue > 0 THEN ABS(t.NetDue) ELSE 0 END, h.ID, @CurrBase, 1, 
		CASE WHEN SIGN(h.TransType) * t.NetDue > 0 THEN 0  ELSE ABS(t.NetDue) END, 
		CASE WHEN SIGN(h.TransType) * t.NetDue > 0 THEN ABS(t.NetDue) ELSE 0 END
	FROM #PsTransList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
		LEFT JOIN dbo.tblArDistCode c ON t.DistCode = c.DistCode
	WHERE h.VoidDate IS NULL AND t.NetDue <> 0

	--Misc payment cash account
	INSERT INTO #TransPostLog (Transdate, [Grouping], Descr, Reference, DistCode, GlAcct, AmountFgn, 
		CreditAmount, DebitAmount, LinkID, CurrencyId, ExchRate, CreditAmtFgn, DebitAmtFgn)   
	SELECT p.PmtDate, 100, @OnAccountInvcNum, p.CustID, t.DistCode, CASE WHEN p.PmtType IN (1, 2, 6) THEN b.GlCashAcct ELSE m.GLAcctDebit END, 
		p.AmountBase, CASE WHEN p.AmountBase > 0 THEN 0  ELSE ABS(p.AmountBase) END, CASE WHEN p.AmountBase > 0 THEN ABS(p.AmountBase) ELSE 0 END, 
		p.ID, @CurrBase, 1, --Standard: only supports base currency external transactions
		CASE WHEN p.AmountBase > 0 THEN 0 ELSE ABS(p.AmountBase) END, CASE WHEN p.AmountBase > 0 THEN ABS(p.AmountBase) ELSE 0 END
	FROM #PsPaymentList t INNER JOIN dbo.tblPsPayment p ON t.ID = p.ID 
		LEFT JOIN dbo.tblArPmtMethod m ON p.PmtMethodID = m.PmtMethodID
		LEFT JOIN dbo.tblSmBankAcct b ON m.BankId = b.BankId 
	...
	INSERT INTO #TransPostLog (Transdate, [Grouping], Descr, Reference, DistCode, GlAcct, AmountFgn, 
		CreditAmount, DebitAmount, LinkID, CurrencyId, ExchRate, CreditAmtFgn, DebitAmtFgn)   
	SELECT p.PmtDate, 110, @OnAccountInvcNum + '/' + 'A/R', p.CustID, t.DistCode, c.GLAcctReceivables, -p.AmountBase, 
		CASE WHEN p.AmountBase > 0 THEN p.AmountBase ELSE 0 END, CASE WHEN p.AmountBase > 0 THEN 0 ELSE ABS(p.AmountBase) END, 
		p.ID, @CurrBase, 1,	CASE WHEN p.AmountBase > 0 THEN p.AmountBase ELSE 0 END, CASE WHEN p.AmountBase > 0 THEN 0 ELSE ABS(p.AmountBase) END
	FROM #PsPaymentList t INNER JOIN dbo.tblPsPayment p ON t.ID = p.ID 
		LEFT JOIN dbo.tblArPmtMethod m ON p.PmtMethodID = m.PmtMethodID
		LEFT JOIN dbo.tblArDistCode c ON t.DistCode = c.DistCode	
	WHERE p.HeaderID IS NULL AND p.VoidDate IS NULL AND p.AmountBase <> 0
	...
	BEGIN
		...
		SELECT @PostRun, @FiscalYear, @FiscalPeriod, [Grouping], GlAcct, -SUM(CreditAmtFgn), 'PS', CASE WHEN [Grouping] = 10 THEN 'Cost of Sales' 
			...
			WHEN [Grouping] = 110 THEN 'A/R' ELSE'Unknown' END, 0,	SUM(CreditAmount), 0, SUM(CreditAmtFgn), @SourceCode, @WrkStnDate, @WrkStnDate, 
			CurrencyId, ExchRate, @CompId
		FROM #TransPostLog 
		...
		SELECT @PostRun, @FiscalYear, @FiscalPeriod, [Grouping], GlAcct, SUM(DebitAmtFgn), 'PS', CASE WHEN [Grouping] = 10 THEN 'Cost of Sales' 
			...
			WHEN [Grouping] = 80 THEN 'Sales' WHEN [Grouping] = 90 THEN 'Sales Tax' WHEN [Grouping] = 100 THEN 'Payments Received' 
			WHEN [Grouping] = 110 THEN 'A/R' ELSE'Unknown' END, SUM(DebitAmount), 0, SUM(DebitAmtFgn), 0, @SourceCode, @WrkStnDate, @WrkStnDate, 
			CurrencyId, ExchRate, @CompId
		FROM #TransPostLog 
		WHERE DebitAmtFgn <> 0
		GROUP BY [Grouping],CurrencyId,ExchRate, GlAcct 
	...
	FROM (SELECT SIGN(h.TransType) * t.InvoiceTotal AS Amount
	FROM #PsTransList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
	...
	FROM #PsPaymentList t INNER JOIN dbo.tblPsPayment p ON t.ID = p.ID 
		INNER JOIN dbo.tblPsTransHeader h ON p.HeaderID = h.ID 
		INNER JOIN #PsTransList s ON h.ID = s.ID
		INNER JOIN dbo.tblArCust c ON h.BillToID = c.CustId
	WHERE p.VoidDate IS NULL AND p.AmountBase <> 0 AND s.NetDue <> 0) o
	...
	SELECT @FiscalYear, @FiscalPeriod, ISNULL(SUM(SIGN(h.TransType) * t.InvoiceTotal),0), ISNULL(@PaymentTotal,0), @CurrBase, ISNULL(@OpenInvoiceTotal,0)
	FROM #PsTransList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PsTransPost_BuildLog_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PsTransPost_BuildLog_proc';
