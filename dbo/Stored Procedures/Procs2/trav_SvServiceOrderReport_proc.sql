...
CREATE PROCEDURE dbo.trav_SvServiceOrderReport_proc
@SortBy int = 0, -- 0 = Location ID, 1 = Work Order Number, 2 = Dispatch Status
...
--		, o.Email, o.OriginalWorkOrder, o.ProjectDetailID, d.ID AS DispatchID, d.DispatchNo, d.[Description]
--		, d.[Status], d.EquipmentDescription, d.RequestedDate, d.RequestedTechID, d.CancelledYN, d.HoldYN
...
--) tmp --{0}
	...
	BEGIN
		SELECT d.WorkOrderID, d.ID AS DispatchID
			, CASE @SortBy 
				WHEN 0 THEN w.SiteID 
				WHEN 1 THEN WorkOrderNo 
				WHEN 2 THEN CASE WHEN @ActivityCount =0 THEN CAST(d.[Status] AS nvarchar) 
					   ELSE CASE WHEN LEN(CAST(ats.Sequence AS nvarchar)) =3 THEN  CAST(ats.Sequence AS nvarchar) 
							...
							END
				END AS GrpId1
			, WorkOrderNo AS ServiceOrderNo, OrderDate, w.SiteID AS LocationID, Address1, Address2
	...
	ELSE
	BEGIN
		SELECT tmp.DispatchID, w.WorkOrderNo AS ServiceOrderNo, d.DispatchNo
			, CASE @SortBy 
				WHEN 0 THEN w.SiteID 
				WHEN 1 THEN WorkOrderNo 
				WHEN 2 THEN CASE WHEN @ActivityCount =0 THEN CAST(d.[Status] AS nvarchar) 
					   ELSE CASE WHEN LEN(CAST(ats.Sequence AS nvarchar)) =3 THEN  CAST(ats.Sequence AS nvarchar) 
							...
							END
				END AS GrpId1
			, d.LocID AS LocationID, r.ActivityDateTime AS ReceivedDateTime
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_SvServiceOrderReport_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_SvServiceOrderReport_proc';
