...
CREATE PROCEDURE dbo.trav_SvWorkOrderPost_SalesRep_proc
AS
BEGIN TRY
	...
	INSERT INTO #tmpCommissions (TransID, SalesRep1Id, SalesRep2Id, InvcDate, TransType, SalesTax, TaxAmtAdj
		...
		SELECT h.TransID, Rep1Id, Rep2Id, InvoiceDate, TransType, SIGN(TransType) * SalesTax, SIGN(TransType) * TaxAmtAdj
			, 0, 0, 0, h.BillToID
			, InvoiceNumber
		...
		WHERE h.VoidYn = 0 AND  h.PrintStatus <> 3 AND (SalesTax <> 0 OR TaxAmtAdj <> 0)
	...
	INSERT INTO #tmpCommissions (TransID, SalesRep1Id, SalesRep2Id, InvcDate, TransType, SalesTax, TaxAmtAdj
		...
		SELECT h.TransID
			...
			, h.InvoiceDate, h.TransType, 0, 0
			, 0, 0, SIGN(TransType) * d.PriceExt, h.BillToID
			, InvoiceNumber
			...
			, CASE WHEN TransType < 0 
				THEN -ROUND(ISNULL(d.QtyUsed * d.UnitCommBasis, d.CostExt), @PrecCurr) 
				ELSE ROUND(ISNULL(d.QtyUsed * d.UnitCommBasis, d.CostExt), @PrecCurr) 
				END
		FROM #PostTransList l INNER JOIN dbo.tblSvInvoiceHeader h on l.TransId = h.TransId
		INNER JOIN dbo.tblSvInvoiceDetail d ON h.TransID = d.TransID 
		WHERE h.VoidYn = 0 AND  h.PrintStatus <> 3 AND (PriceExt <> 0 OR ISNULL(d.QtyUsed * d.UnitCommBasis, d.CostExt) <> 0)
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_SvWorkOrderPost_SalesRep_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_SvWorkOrderPost_SalesRep_proc';
