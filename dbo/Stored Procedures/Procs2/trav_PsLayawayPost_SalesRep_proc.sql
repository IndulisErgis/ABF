...
CREATE PROCEDURE dbo.trav_PsLayawayPost_SalesRep_proc
AS
SET NOCOUNT ON
BEGIN TRY
	--Rounding adjustment is not part of commission invoice total
	--TODO, Reward Redemption
	...
	INSERT INTO #tmpCommissions (TransID, SalesRepId, InvcDate, SalesTax, Freight, Misc, PriceExt, CustId, InvcNum, AmtCogs) 
	SELECT h.ID, h.SalesRepID, h.CompletedDate, d.SalesTax, d.Freight, d.Misc, 0, h.BillToID, t.InvoiceNum, 0 
	...
	INSERT INTO #tmpCommissions (TransID, SalesRepId, InvcDate, SalesTax, Freight, Misc, PriceExt, CustId, InvcNum, AmtCogs) 
	SELECT h.ID, ISNULL(d.SalesRepID, h.SalesRepID), h.CompletedDate, 0, 0, 0, d.ExtPrice - ISNULL(c.ExtPrice,0), 
		h.BillToID, t.InvoiceNum, ISNULL(i.ExtCost,0)
	FROM #PsCompletedLayawayList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
	...
	INSERT INTO #tmpCommissions (TransID, SalesRepId, InvcDate, SalesTax, Freight, Misc, PriceExt, CustId, InvcNum, AmtCogs) 
	SELECT h.ID, h.SalesRepID, h.CompletedDate, 0, 0, 0, c.ExtPrice, h.BillToID, t.InvoiceNum, 0 
	FROM #PsCompletedLayawayList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
	...
	FROM dbo.tblArSalesRep 
		INNER JOIN (SELECT SalesRepId, MAX(InvcDate) InvcDate
	...
	BEGIN
		...
		BEGIN
			...
			SELECT s.SalesRepID, t.CustId, t.InvcNum, t.InvcDate, 100, s.CommRate, s.PctOf, s.BasedOn, s.PayOnLineItems, 
				s.PayOnSalesTax, s.PayOnFreight, s.PayOnMisc, t.PriceExt, t.SalesTax, t.Freight, t.Misc, t.AmtCogs, d.InvoiceTotal, d.Payment 
		...
		BEGIN
			...
			SELECT s.SalesRepID, h.BillToID, h.TransIDPrefix + CAST(h.TransID AS varchar), h.TransDate, 100, s.CommRate, 
				...
				d.ExtPrice, d.Tax, d.Freight, d.Misc, ISNULL(d.ExtCost,0), d.InvoiceTotal, d.Payment
			FROM #PsCompletedLayawayList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PsLayawayPost_SalesRep_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PsLayawayPost_SalesRep_proc';
