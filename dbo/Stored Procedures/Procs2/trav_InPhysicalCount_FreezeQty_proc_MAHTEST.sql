CREATE PROCEDURE [dbo].[trav_InPhysicalCount_FreezeQty_proc_MAHTEST]   
@BatchId pBatchId  
AS  
...
SELECT @PrecQty = 2 
SELECT @PrecCurr = 2 
SELECT @CostingMethod = 2
SELECT @ExcludePickedQuantity = 0 
...
BEGIN  
 ...
 --mah test:
 select 'inner join with lot view', QtyFrozen = q.QtyOnHand   
 From dbo.tblInPhysCount INNER JOIN dbo.trav_InItemOnHandLot_view q   
  ON dbo.tblInPhysCount.ItemId = q.ItemId And dbo.tblInPhysCount.LocId = q.LocId And (dbo.tblInPhysCount.LotNum = q.LotNum OR (dbo.tblInPhysCount.LotNum IS NULL AND q.LotNum IS NULL))   
  INNER JOIN dbo.tblInItemLoc l ON q.ItemId = l.ItemId And q.LocId = l.LocId   
 WHERE dbo.tblInPhysCount.BatchId = @BatchId  
...
BEGIN  
 ...
 --UPDATE dbo.tblInPhysCount SET QtyFrozen = q.QtyOnHand  
 SELECT 'null extended loc in header', QtyFrozen = q.QtyOnHand  
 ...
 SELECT 'extended locations in the detail', QtyFrozen = q.QtyOnHand   
...
select 'when details',  TotalQtyFrozen = QtyFrozen + ISNULL(d.QtyFrozenDetail,0)  
...
-- select CostFrozen = ROUND(TotalQtyFrozen * l.CostAvg,@PrecCurr)  
