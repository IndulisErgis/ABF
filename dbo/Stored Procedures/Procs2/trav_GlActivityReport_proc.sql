...
CREATE PROCEDURE dbo.trav_GlActivityReport_proc
...
BEGIN TRY
	...
	(
		AcctId pGlAcct NOT NULL,
 		...
 		[Year] smallint NOT NULL, 
		SumOfActual pCurrDecimal NOT NULL, 
		EndingBal pCurrDecimal NOT NULL, 
	...
	FROM #tmpAccountList t INNER JOIN dbo.tblGlAcctHdr h ON t.AcctId = h.AcctId 
		LEFT JOIN (SELECT AcctId, SUM(Actual) Actual FROM dbo.tblGlAcctDtl 
			WHERE [Year] = @YearFrom AND Period < @PeriodFrom 
			GROUP BY AcctId) d ON t.AcctId = d.AcctId
	...
	(SELECT d.AcctID, SUM(CASE WHEN h.BalType < 0 Then -(d.DebitAmtFgn - d.CreditAmtFgn) ELSE (d.DebitAmtFgn - d.CreditAmtFgn) END) AS Unposted
	FROM #tmpAccountList t INNER JOIN dbo.tblGlAcctHdr h ON t.AcctId = h.AcctId 
	...
	GROUP BY d.AcctID,h.BalType) d ON t.AcctID=d.AcctID
	...
	INSERT INTO #temp3 (AcctId, Period, [Year], SumOfActual, Adjustment, EndingBal)
	SELECT t.AcctId, p.Period, p.[Year], t.SumOfActual, 0, 0 
	...
	BEGIN
		...
		SELECT t.AcctId, j.Period, j.[Year], 
			CASE WHEN BalType >= 0 THEN SUM(DebitAmtFgn) - SUM(CreditAmtFgn) ELSE SUM(CreditAmtFgn) - SUM(DebitAmtFgn) END 
		FROM #temp1 t INNER JOIN tblGlJrnl j (nolock) ON t.AcctId = j.AcctId 
			INNER JOIN #tempPdYear p ON j.[Year] = p.[Year] AND j.Period = p.Period 
		WHERE  j.PostedYn = -1 
	...
	SET @cursor1 = CURSOR FOR 
		SELECT ([Year] * 1000) + Period AS PeriodYear FROM #tempPdYear FOR READ ONLY
	OPEN @cursor1
	IF @@CURSOR_ROWS <> 0
	BEGIN
		FETCH NEXT FROM @cursor1 INTO @YearPeriod
		WHILE (@@FETCH_STATUS = 0)
		BEGIN
		UPDATE #temp3 SET EndingBal = SumOfActual + ISNULL(a.Adjustment,0) + ISNULL(t.JournalTotal,0) 
		FROM #temp3 
			LEFT JOIN 
			(
				SELECT d.AcctID
					, SUM(CASE WHEN h.BalType < 0 
						THEN -(d.DebitAmtFgn - d.CreditAmtFgn) 
						ELSE (d.DebitAmtFgn - d.CreditAmtFgn) END) AS JournalTotal 
				FROM #tmpAccountList t 
					INNER JOIN dbo.tblGlAcctHdr h ON t.AcctId = h.AcctId 
					INNER JOIN dbo.tblGlJrnl d ON h.AcctId = d.AcctId 
				WHERE ([Year] * 1000) + Period <= @YearPeriod AND ([Year] * 1000) + Period >= (@YearFrom * 1000) + @PeriodFrom
				AND h.BalType <> 0 
					AND SourceCode BETWEEN @SourceCodeFrom AND @SourceCodeThru 
				GROUP BY d.AcctID,h.BalType
			) t ON #temp3.AcctId = t.AcctId 
			LEFT JOIN 
			(
				SELECT AcctId, SUM(Adjustment) Adjustment 
				FROM #temp3 
				WHERE ([Year] * 1000) + Period <= @YearPeriod AND ([Year] * 1000) + Period >= (@YearFrom * 1000) + @PeriodFrom
				GROUP BY AcctId
			) a ON #temp3.AcctId = a.AcctId 
		WHERE(#temp3.[Year] * 1000) + #temp3.Period = @YearPeriod
	...
	SELECT t.[Year], s.SortOrder
		...
		, CASE WHEN h.BalType < 0 THEN -EndingBal ELSE EndingBal END AS EndingBal
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_GlActivityReport_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_GlActivityReport_proc';
