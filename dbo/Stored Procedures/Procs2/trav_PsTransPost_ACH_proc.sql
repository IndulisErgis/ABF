...
CREATE PROCEDURE dbo.trav_PsTransPost_ACH_proc
AS
BEGIN TRY
	...
	FROM #PsPaymentList t INNER JOIN dbo.tblPsPayment p ON t.ID = p.ID 
		...
		INNER JOIN dbo.tblSmBankAcct b ON m.BankId = b.BankId 
		LEFT JOIN dbo.tblArCust c ON p.CustID = c.CustId
	WHERE p.HeaderID IS NULL AND p.VoidDate IS NULL AND p.PmtType = 6 AND p.AmountBase <> 0 --direct debits only
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PsTransPost_ACH_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PsTransPost_ACH_proc';
