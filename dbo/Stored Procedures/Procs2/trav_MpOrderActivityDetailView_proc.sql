...
CREATE procedure  [dbo].[trav_MpOrderActivityDetailView_proc]     
...
if(@TransType IN (1,2,4,6))  
	...
	BEGIN      
		SELECT d.TransId, r.ReqId, d.TransDate, d.ComponentId, i.Descr, d.LocId, d.UnitCost, (ISNULL(s.EstQtyRequired,0)* ISNULL(su.ConvFactor,1))/ISNULL(du.ConvFactor,1) AS EstQty, (ISNULL(s.EstScrap,0)* ISNULL(su.ConvFactor,1))/ISNULL(du.ConvFactor,1) AS EstScrap, d.Qty AS ActualQty,d.ActualScrap AS ActualScrap, d.UOM, d.GlPeriod, d.FiscalYear,  
...
Else if(@TransType=3)
	...
	BEGIN    
		SELECT d.TransId, r.ReqId, d.TransDate, s.OperationId, s.LaborTypeId, (d.LaborSetup/d.LaborSetupIn) AS ActLaborSetUp,    
			   ...
			   d.QtyScrapped, d.VarianceCode, r.ReqId, s.LaborSetupTypeId    
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_MpOrderActivityDetailView_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_MpOrderActivityDetailView_proc';
