...
CREATE PROCEDURE dbo.trav_PcDailySalesTaxJournal_proc
AS
SET NOCOUNT ON
BEGIN TRY

	CREATE TABLE #tmpPcDailyTax
	(
	...
	INSERT INTO #tmpPcDailyTax (TaxLocId, TaxClass, TaxableAmt, NontaxableAmt, TaxCollected, TaxCalculated) 
	...
	FROM #tmpInvoiceList m INNER JOIN dbo.tblPcInvoiceHeader h ON m.TransId = h.TransId
		INNER JOIN dbo.tblPcInvoiceTax t ON h.TransId = t.TransId
		INNER JOIN #tmpTaxLocationList l ON t.TaxLocID = l.TaxLocID 
	...
	FROM #tmpInvoiceList m INNER JOIN dbo.tblPcInvoiceHeader h ON m.TransId = h.TransId
		INNER JOIN #tmpTaxLocationList l ON h.TaxLocAdj = l.TaxLocID
	...
	FROM dbo.tblSmTaxLoc t 
		INNER JOIN (dbo.tblSmTaxClass c 
			...
			INNER JOIN #tmpPcDailyTax x ON (d.TaxClassCode = x.TaxClass) 
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PcDailySalesTaxJournal_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PcDailySalesTaxJournal_proc';
