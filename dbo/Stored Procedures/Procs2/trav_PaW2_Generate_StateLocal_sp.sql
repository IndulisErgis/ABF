...
CREATE PROCEDURE [dbo].[trav_PaW2_Generate_StateLocal_sp]
@PaYear smallint
AS
BEGIN TRY
...
declare @FicaLimit decimal(20,10)
DECLARE
@BoxC   Varchar(50),
@BoxB      Varchar(17),
@BoxC1   Varchar(70),
...
Declare @TaxId varchar(17)
Declare @Local char(2)
Declare @Code pCode
Declare @StateTaxableYTD pDec
Declare @StateWithholdYTD pDec
Declare @LocalDescr varchar(30)
Declare @LocalTaxableYTD pDec
Declare @LocalWithholdYTD pDec
...
(
	...
	State char(2),
	TaxId varchar(17),
	TaxableYTD pDec, --Box17
	WithholdYTD pDec --Box18
)
...
(
	...
	Local char(2),
	Code pCode,
	Description varchar(30), --Box19/19a
	TaxableYTD pDec, --Box20/20a
	WithholdYTD pDec --Box21/21a
)
...
(
	...
	EmployeeID pEmpId,
	State char(2), --Box16a
	TaxId varchar(17), --Box16b
	StateTaxableYTD pDec, --Box17
	StateWithholdYTD pDec, --Box18
	LocalCodeDescr1 varchar(30), --Box19
	LocalTaxableYTD1 pDec, --Box20
	LocalWithholdYTD1 pDec, --Box21
	LocalCodeDescr2 varchar(30), --Box19a
	LocalTaxableYTD2 pDec, --Box20a
	LocalWithholdYTD2 pDec --Box21a
)
...
WHEN 'G'  THEN Box13line1 WHEN 'H'  THEN Box13line1 WHEN 'S'  THEN Box13line1 WHEN 'Y'  THEN Box13line1 WHEN 'AA'  THEN Box13line1 WHEN 'BB' THEN Box13line1 ELSE 0 end)
 + SUM(CASE  Box13lineDesc2 WHEN 'D'  THEN Box13line2 WHEN 'E' THEN Box13line2  WHEN 'F' THEN Box13line2
WHEN 'G'  THEN Box13line2 WHEN 'H'  THEN Box13line2 WHEN 'S'  THEN Box13line2 WHEN 'Y'  THEN Box13line2 WHEN 'AA'  THEN Box13line2 WHEN 'BB' THEN Box13line2 ELSE 0 end)
 + SUM(CASE  Box13lineDesc3 WHEN 'D'  THEN Box13line3 WHEN 'E' THEN Box13line3  WHEN 'F' THEN Box13line3
WHEN 'G'  THEN Box13line3 WHEN 'H'  THEN Box13line3 WHEN 'S'  THEN Box13line3  WHEN 'Y'  THEN Box13line3 WHEN 'AA'  THEN Box13line3 WHEN 'BB' THEN Box13line3 ELSE 0 end)
+ SUM(CASE  Box13lineDesc4 WHEN 'D'  THEN Box13line4 WHEN 'E' THEN Box13line4  WHEN 'F' THEN Box13line4
WHEN 'G'  THEN Box13line4 WHEN 'H'  THEN Box13line4 WHEN 'S'  THEN Box13line4 WHEN 'Y'  THEN Box13line4 WHEN 'AA'  THEN Box13line4 WHEN 'BB' THEN Box13line4 ELSE 0 end),
SUM(Box17) ,SUM(Box18) , 'STATE TOTALS' ,CONVERT(Varchar(10),COUNT(DISTINCT employeeid)) + ' EMPLOYEES' 
FROM dbo.tblPaW2 GROUP BY Box16a


--generate control numbers ordered by state and employee
declare --@TaxAuth varchar(2), @Emp varchar(11), 
@Count int, 
@Lastname varchar(20)
set @Count=0
...
'GRAND TOTALS', CONVERT(Varchar(10),COUNT(DISTINCT employeeid)) + ' EMPLOYEES' 
FROM dbo.tblPaW2
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.10203.1229', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PaW2_Generate_StateLocal_sp';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 13344', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PaW2_Generate_StateLocal_sp';
