...
CREATE PROCEDURE dbo.trav_GlPeriodicAllocations_ResolveAccountsReport_proc
AS
SET NOCOUNT ON
BEGIN TRY
	...
	Insert into #SegmentInfo ([Number], [Length], [Start],Description)
	...
	Insert into #ResolvedAccounts ( [CalcStatus], [Status]
		...
		, [AllocType], [AllocAmount], [AllocLimit],[SourceType],RecipientType,AlolcationDescription)
	Select
		  Case When pc.[Status] <> 0  Then 256 Else 0 End --identify codes that are invalid
		, 0, pd.[DetailType], pc.ID, pd.Sequence, 0, pd.[Sequence]
		, pc.[DistType], pc.[AllocBasis], NULL, NULL, pd.[AccountID], ISNULL(pd.[AltAccountID], pd.[AccountID])
		, ISNULL(pd.[AllocType], 0), ISNULL(pd.[AllocAmount], 0), ISNULL(pd.[AllocLimit], 0), pc.SourceType,0 ,'Account'
		...
		WHERE (pc.[SourceType] = 0 AND pd.[DetailType] = 0)  --Full Account ID source
	...
	Select 
		...
		, 0, pd.[DetailType], pc.ID, pd.[Sequence], 0, pd.[Sequence]
		, pc.[DistType], pc.[AllocBasis], NULL, NULL, pd.[AccountID], ISNULL(pd.[AltAccountID], pd.[AccountID])
		, ISNULL(pd.[AllocType], 0), ISNULL(pd.[AllocAmount], 0), ISNULL(pd.[AllocLimit], 0), 0,pc.RecipientType,'Account'
		From dbo.tblGlAllocPdCode pc  
		Inner Join #GroupIDList tg on pc.[ID] = tg.[GroupID]		
		Inner Join dbo.tblGlAllocPdCodeDetail pd on pc.[ID] = pd.[AllocCodeID]		
		WHERE (pc.[RecipientType] = 0 AND pd.[DetailType] = 1) --Full Account ID receipient
	...
	Insert into #ResolvedAccounts ( [CalcStatus], [Status]
		...
		, [DistType], [AllocBasis], [SegmentId], [AltSegmentId], [AccountId], [AltAccountId]
		, [AllocType], [AllocAmount], [AllocLimit], [SourceType],RecipientType,AlolcationDescription)
	Select Case When pc.[Status] <> 0  Then 256 Else 0 End --identify codes that are invalid
		, 0, ps.[DetailType], pc.ID, pd.[Sequence], ps.[Sequence], pd.[Sequence]
		...
		, pc.SourceType,0,si.Description
		From dbo.tblGlAllocPdCode pc
		Inner Join #GroupIDList tg on pc.[ID] = tg.[GroupID]		
	...
	Select Case When pc.[Status] <> 0  Then 256 Else 0 End --identify codes that are invalid
		...
		, Case When NULLIF(pd.[AllocType], 0) IS NULL --use segment when detail type is null or zero or detail amount is zero and types match
			...
			End
		, COALESCE(NULLIF(pd.[AllocLimit], 0), ps.[AllocLimit], 0) --use segment value when detail is null or zero
		,0, pc.RecipientType, si.Description
		From dbo.tblGlAllocPdCode pc 
		Inner Join #GroupIDList tg on pc.[ID] = tg.[GroupID]		
		Inner Join dbo.tblGlAllocPdCodeSegment ps on pc.[ID] = ps.[AllocCodeID]
		Inner Join dbo.tblGlAllocPdCodeDetail pd on ps.[AllocCodeID] = pd.[AllocCodeID] and ps.[DetailType] = pd.[DetailType] --cross join between Segment and Detail
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_GlPeriodicAllocations_ResolveAccountsReport_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_GlPeriodicAllocations_ResolveAccountsReport_proc';
