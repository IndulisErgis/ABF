...
CREATE PROCEDURE dbo.trav_PsTransPost_SalesHistory_proc
AS
SET NOCOUNT ON
BEGIN TRY

	DECLARE @PostRun pPostRun, @WrkStnDate datetime, @FiscalYear smallint, @FiscalPeriod smallint,
		@PrecUCost tinyint, @OnAccountInvcNum nvarchar(15), @PrecUPrice tinyint, @PrecQty tinyint, @CurrBase pCurrency
	...
	IF @PostRun IS NULL OR @WrkStnDate IS NULL OR @FiscalYear IS NULL OR @FiscalPeriod IS NULL OR @PrecUCost IS NULL 
		OR @OnAccountInvcNum IS NULL OR @PrecUPrice IS NULL OR @PrecQty IS NULL OR @CurrBase IS NULL
	BEGIN
	...
	FROM dbo.tblArCust c INNER JOIN (SELECT h.BillToID FROM #PsTransList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
	...
	SELECT @PostRun, t.TransID, SIGN(h.TransType), NULL, h.BillToID, h.ShipToID, c.Name, c.Attn, c.Address1, c.Address2, c.City, c.Region, 
		c.Country, c.PostalCode, c.Phone, h.ShipVia, u.TermsCode, h.TaxableYN, t.InvoiceNum, NULL,t.LocID, h.TransDate, h.ShipNum, 
		h.TransDate, h.TransDate, h.SalesRepID, 100, r.CommRate, NULL, 0, 0, 1, 0, 0, @WrkStnDate, @FiscalPeriod, @FiscalYear, h.TaxGroupID, 
		x.Taxable, x.NonTaxable, ISNULL(d.Tax,0), 0, 0, ISNULL(d.ExtCost,0)
		, x.Taxable, x.NonTaxable, ISNULL(d.Tax,0), 0, 0, ISNULL(d.ExtCost,0)
		, 2, h.PONumber, t.DistCode, h.CurrencyID, 1, t.DiscDueDate, ISNULL(t.NetDueDate,h.TransDate), ISNULL(t.DiscAmt,0), ISNULL(t.DiscAmt,0), @FiscalPeriod, 0, 0, NULL, NULL, NULL
		, 0, h.SoldToID, CustLevel, h.PODate, ISNULL(h.ReqShipDate, h.TransDate), NULL, NULL, NULL, 4, NULL, h.SourceId, 1, o.GLAcctReceivables, NULL, o.GLAcctFreight, o.GLAcctMisc
		, NULL, CASE WHEN h.VoidDate IS NULL THEN 0 ELSE 1 END, h.Notes, ISNULL(d.Payment, 0), ISNULL(d.Payment, 0), 0, h.CF
	...
	SELECT @PostRun, t.TransID, CASE d.LineType WHEN 3 THEN -2 WHEN 4 THEN -3 END, 
		0, t.LocID, 0, d.TaxClass, CASE d.LineType WHEN 3 THEN c.GLAcctFreight WHEN 4 THEN c.GLAcctMisc END,
		1, 1, 1, 1, d.ExtPrice, d.ExtPrice, d.ExtPrice, d.ExtPrice, d.ExtPrice, d.ExtPrice, h.TransDate, h.TransDate, h.SalesRepID, 100, r.CommRate, NULL, 
		0, 0, CASE d.LineType WHEN 3 THEN 'Freight' WHEN 4 THEN 'Misc' END, NULL
	FROM #PsTransList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
	...
	SELECT @PostRun, t.TransID, d.EntryNum, d.LineSeq, d.LocId, d.ItemId, NULL, NULL, NULL, d.[Descr], CASE WHEN i.ItemId IS NULL THEN 0 ELSE i.ItemType END, 
		...
		NULL, 0, NULL, NULL, NULL, d.ResCode, 0, ISNULL(u.ConvFactor, 1), d.PromoID, NULL, d.Qty, NULL, d.CF
	FROM #PsTransList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
	...
	INSERT INTO dbo.tblArHistDetail (PostRun, TransId, EntryNum, LineSeq, WhseId, PartType, TaxClass, GLAcctSales
		...
		, ReqShipDate, ActShipDate, Rep1Id, Rep1Pct, Rep1CommRate, Rep2Id, Rep2Pct, Rep2CommRate, ResCode)
	SELECT @PostRun, t.TransID, d.EntryNum, d.LineSeq, t.LocID, 0, d.TaxClass, CASE WHEN h.VoidDate IS NULL THEN NULL ELSE CASE d.LineType WHEN -2 THEN c.GLAcctCoupon 
		...
		h.TransDate, h.TransDate, h.SalesRepID, 100, r.CommRate, NULL, 0, 0, d.ResCode
	FROM #PsTransList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
	...
	SELECT @PostRun, @WrkStnDate, 0, p.CustID, t.TransID, NULL, @OnAccountInvcNum, p.CheckNum, p.CcNum, p.CcHolder, p.CcAuth, p.CcExpire, m.BankId, p.BankName,
		...
		NULL, NULL, t.DistCode, 0, p.Notes, NEWID(), CASE WHEN p.VoidDate IS NULL THEN 0 ELSE 1 END, p.CF
	...
	--Append payment of transaction that does not have rounding adjustment
	--or non-cash payment of transaction
	INSERT dbo.tblArHistPmt (PostRun, PostDate, RecType, CustId, TransId, DepNum, InvcNum, CheckNum, CcNum, CcHolder, CcAuth, CcExpire, BankID, BankName, 
		BankRoutingCode, BankAcctNum, PmtMethodId, CurrencyId, ExchRate, PmtDate, PmtAmt, DiffDisc, PmtAmtFgn, DiffDiscFgn, PmtType, SumHistPeriod, GLPeriod, 
		FiscalYear, GLRecvAcct, GlAcctGainLoss, GlAcctDebit, Rep1Id, Rep2Id, DistCode, CalcGainLoss, Note, SourceId, VoidYn, CF) 
	...
	FROM #PsPaymentList t INNER JOIN dbo.tblPsPayment p ON t.ID = p.ID 
		...
		LEFT JOIN dbo.tblArPmtMethod m ON p.PmtMethodID = m.PmtMethodID 
		LEFT JOIN dbo.tblSmBankAcct b ON m.BankId = b.BankId  
		LEFT JOIN (SELECT t.ID FROM #PsTransList t INNER JOIN dbo.tblPsTransDetail d ON t.ID = d.HeaderID WHERE LineType = -4 AND ExtPrice <> 0 GROUP BY t.ID) r 
			ON h.ID = r.ID
	WHERE p.VoidDate IS NULL AND (p.PmtType <> 1 OR r.ID IS NULL)  

	--append cash payment of transaction that has rounding adjustment
	...
	FROM #PsPaymentList t INNER JOIN dbo.tblPsPayment p ON t.ID = p.ID 
		INNER JOIN dbo.tblPsTransHeader h ON p.HeaderID = h.ID 
		INNER JOIN #PsTransList i ON h.ID = i.ID
		INNER JOIN (SELECT t.ID, SUM(ExtPrice) AS RoudingAdj FROM #PsTransList t INNER JOIN dbo.tblPsTransDetail d ON t.ID = d.HeaderID WHERE LineType = -4 AND ExtPrice <> 0 GROUP BY t.ID) r 
			ON h.ID = r.ID 
		LEFT JOIN (SELECT l.ID, MAX(p.EntryDate) LastPmtDate FROM #PsPaymentList t INNER JOIN dbo.tblPsPayment p ON t.ID = p.ID --Last cash payment
			INNER JOIN #PsTransList l ON p.HeaderID = l.ID 
			WHERE p.PmtType = 1
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PsTransPost_SalesHistory_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PsTransPost_SalesHistory_proc';
