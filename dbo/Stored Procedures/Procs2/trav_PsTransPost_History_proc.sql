...
CREATE PROCEDURE dbo.trav_PsTransPost_History_proc
AS
SET NOCOUNT ON
BEGIN TRY
	--TODO: Layaway
	...
	SELECT h.ID, h.TransIDPrefix, h.TransID, h.TransType, h.TransDate, h.SoldToID, h.BillToID, h.ShipToID, h.ShipVia, h.ShipNum, h.TaxableYN, 
		...
		h.CF, @PostRun, t.DistCode, t.LocID, c.GLAcctReceivables, h.RewardNumber, h.CompletedDate, h.PONumber, h.PODate, h.ReqShipDate, h.iCap
	...
	INSERT INTO dbo.tblPsHistPayment (ID, HeaderID, PmtDate, CustID, PmtType, PmtMethodID, CcNum, CcHolder, CcExpire, CcAuth, CcRef, CheckNum, 
		...
		DistCode, GLAcctCash, GLAcct, Response, iCap)
	SELECT p.ID, p.HeaderID, p.PmtDate, p.CustID, p.PmtType, p.PmtMethodID, p.CcNum, p.CcHolder, p.CcExpire, p.CcAuth, p.CcRef, p.CheckNum, p.BankName, 
		p.BankRoutingCode, p.BankAcctNum, p.Amount, p.AmountBase, p.CurrencyID, p.VoidDate, p.UserID, p.HostID, p.EntryDate, p.Notes, p.CF, @PostRun, 
		t.LocID, t.DistCode, CASE WHEN p.PmtType IN (1, 2, 6) THEN b.GlCashAcct ELSE m.GLAcctDebit END, --Use the bank gl account for Cash, Check and Direct Debit 
		c.GLAcctReceivables, p.Response, p.iCap
	...
	SELECT p.ID, p.HeaderID, p.PmtDate, p.CustID, p.PmtType, p.PmtMethodID, p.CcNum, p.CcHolder, p.CcExpire, p.CcAuth, p.CcRef, p.CheckNum, 
		...
		p.CF, @PostRun, t.LocID, i.DistCode, 
	...
	FROM #PsTransList t INNER JOIN dbo.tblPsTransContact c ON t.ID = c.HeaderID
	...
	FROM #PsTransList t INNER JOIN dbo.tblPsTransTax x ON t.ID = x.HeaderID 
	...
	INSERT INTO dbo.tblPsHistDetail (ID, HeaderID, ParentID, EntryNum, LineSeq, LineType, ItemID, LocID, LotNum, SerNum, Descr, TaxClass, 
		Qty, Unit, ExtPrice, TaxAmount, PromoID, SalesRepID, Notes, CF, GLAcct, GLAcctCOGS, GLAcctInv, ResCode)
	...
	SELECT d.ID, d.HeaderID, d.ParentID, d.EntryNum, d.LineSeq, d.LineType, d.ItemID, d.LocID, d.LotNum, d.SerNum, d.Descr, d.TaxClass, 
		...
		WHEN -4 THEN e.GLAcctRounding END, NULL, NULL, d.ResCode
	FROM #PsTransList t INNER JOIN dbo.tblPsTransHeader h ON t.ID = h.ID 
		INNER JOIN dbo.tblPsTransDetail d ON h.ID = d.HeaderID 
		LEFT JOIN dbo.tblArDistCode c ON t.DistCode = c.DistCode
	...
	UNION ALL
	SELECT d.ID, d.HeaderID, d.ParentID, d.EntryNum, d.LineSeq, d.LineType, d.ItemID, d.LocID, d.LotNum, d.SerNum, d.Descr, d.TaxClass, 
		d.Qty, d.Unit, d.ExtPrice, d.TaxAmount, d.PromoID, d.SalesRepID, d.Notes, d.CF, NULL, NULL, NULL,d.ResCode
	FROM #PsTransList t INNER JOIN dbo.tblPsTransDetail d ON t.ID = d.HeaderID 
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PsTransPost_History_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PsTransPost_History_proc';
