...
CREATE PROCEDURE dbo.trav_PcBillingPost_SalesRep_proc
AS
BEGIN TRY
	...
	INSERT INTO #tmpCommissions (TransID, SalesRep1Id, SalesRep2Id, InvcDate, TransType, SalesTax, TaxAmtAdj
		...
		SELECT h.TransID, Rep1Id, Rep2Id, InvcDate, TransType, SIGN(TransType) * SalesTax, SIGN(TransType) * TaxAmtAdj
			...
			, CASE WHEN TransType < 0 
				THEN ISNULL(OrgInvcNum, ISNULL(h.InvcNum, l.DefaultInvoiceNumber)) 
		...
		WHERE h.VoidYn = 0 AND (SalesTax <> 0 OR TaxAmtAdj <> 0)
	...
	INSERT INTO #tmpCommissions (TransID, SalesRep1Id, SalesRep2Id, InvcDate, TransType, SalesTax, TaxAmtAdj
		...
		SELECT h.TransID
			...
			, 0, 0, SIGN(TransType) * d.ExtPrice, h.CustId
			, CASE WHEN TransType < 0 
				THEN ISNULL(OrgInvcNum, ISNULL(h.InvcNum, l.DefaultInvoiceNumber)) 
				ELSE ISNULL(h.InvcNum, l.DefaultInvoiceNumber) 
				END
			...
			, CASE WHEN TransType < 0 
				THEN -ROUND(ISNULL(d.Qty * d.UnitCommBasis, d.ExtCost), @PrecCurr) 
				ELSE ROUND(ISNULL(d.Qty * d.UnitCommBasis, d.ExtCost), @PrecCurr) 
				END
		FROM #PostTransList l INNER JOIN dbo.tblPcInvoiceHeader h on l.TransId = h.TransId
		INNER JOIN dbo.tblPcInvoiceDetail d ON h.TransID = d.TransID 
		WHERE h.VoidYn = 0 AND (ExtPrice <> 0 OR ISNULL(d.Qty * d.UnitCommBasis, d.ExtCost) <> 0)
	...
	BEGIN
		...
		UPDATE dbo.tblArCommInvc 
			SET AmtPmt = AmtPmt + pmt.PmtAmt
		FROM dbo.tblArCommInvc
		INNER JOIN (SELECT h.CustId, CASE WHEN TransType < 0 
				THEN ISNULL(OrgInvcNum, ISNULL(h.InvcNum, l.DefaultInvoiceNumber)) 
				ELSE ISNULL(h.InvcNum, l.DefaultInvoiceNumber) 
				END InvcNum
			, Sum(SIGN(h.TransType) * d.DepositAmtApply) PmtAmt
			FROM #PostTransList l INNER JOIN dbo.tblPcInvoiceHeader h on l.TransId = h.TransId 
				INNER JOIN dbo.tblPcInvoiceDeposit d ON h.TransId = d.TransId
			WHERE h.VoidYn = 0
			GROUP BY h.CustId, CASE WHEN TransType < 0 
				THEN ISNULL(OrgInvcNum, ISNULL(h.InvcNum, l.DefaultInvoiceNumber)) 
				ELSE ISNULL(h.InvcNum, l.DefaultInvoiceNumber) 
				END
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PcBillingPost_SalesRep_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_PcBillingPost_SalesRep_proc';
