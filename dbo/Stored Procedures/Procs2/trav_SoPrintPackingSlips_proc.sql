...
CREATE PROCEDURE dbo.trav_SoPrintPackingSlips_proc
@LastSlipNumber pInvoiceNum = null, --set for batch mode printing
@PrintType tinyint = 0, --bitmasked flag to identify the type of transactions to process for batch mode printing (Invoice = 1, Verified = 2)
...
BEGIN TRY
	...
	INSERT INTO #tmpSoInvoice (TransType, PrintType) VALUES (1,1) --invoice
	...
	BEGIN
		...
		WHERE h.VoidYn = 0 --exclude voids
			AND (h.PrintPackStatus <> 1) 
			AND (h.PackNum > @LastSlipNumber OR h.PackNum Is Null) 
	...
	--PET:http://traversedev.internal.osas.com:8090/pets/view.php?id=12791
	...
	SELECT c.CustId, c.Attn, c.Contact, c.CustName, c.Addr1, c.Addr2, c.City, c.Region
		...
		, NULL PackingListNo --placeholder for the generated Packing List No
	...
	SELECT d.ItemId, d.EntryNum, d.LocId AS LocationId
		...
		, d.BinNum AS BinNo, d.BOLNum, d.Rep1Id, d.Rep2Id, ISNULL(t.DropShipYn, 0) AS DropShipYn, d.CustomerPartNumber  
	FROM #Temp l
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_SoPrintPackingSlips_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_SoPrintPackingSlips_proc';
