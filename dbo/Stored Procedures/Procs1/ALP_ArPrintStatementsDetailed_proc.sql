...
CREATE PROCEDURE [dbo].[ALP_ArPrintStatementsDetailed_proc]   
@CutoffDate datetime,           
...
BEGIN TRY       
 ...
 --MAH 01/25/2015 several changes to control display of credit memos and payments against invoices    
 ...
 --CREATE TABLE #CustomerList (CustId pCustId, PRIMARY KEY (CustId))        
 --INSERT INTO #CustomerList (CustId) SELECT CustID from dbo.tblArCust where CustID = '39016' --GroupCode = 'C' OR GroupCode = 'A'--          
 -- --42507, 65.00042     
...
SET @CompanyName = (
					SELECT
						Name
					FROM
						SYS.dbo.tblSmCompInfo
					WHERE 
						SYS.dbo.tblSmCompInfo.CompID = db_name()
					)      
 --MAH - TEMPORARY! - use parameter to control display and aging of only open invoices      
DECLARE @IncludeOnlyOpenInvoices tinyint      
SET @IncludeOnlyOpenInvoices  = 1          
...
(          
	...
	INDEX IX_#AgeInvoiceList_CustId_InvcNum NONCLUSTERED (CustId, InvcNum)
)
...
(          
	...
	INDEX IX_#AgeInvoicelistBySite_AlpSiteId NONCLUSTERED (AlpSiteId)
)              
...
(          
	...
	PRIMARY KEY ([Counter]),
	INDEX IX_#InvoiceActivity_CustId NONCLUSTERED (CustId)
)          
...
INSERT INTO
	...
	SELECT
		...
		AcctType, 
		0, 
		ISNULL(c.NewFinch, 0) + ISNULL(c.UnpaidFinch, 0), 
		ISNULL(c.CurAmtDue, 0), 
		ISNULL(c.BalAge1, 0), 
		ISNULL(c.BalAge2, 0), 
		ISNULL(c.BalAge3, 0), 
		ISNULL(c.BalAge4, 0), 
		ISNULL(c.UnapplCredit, 0),
		ISNULL(c.PostalCode,' ' ), 
		ISNULL(c.GroupCode,' ')        
	FROM
		#CustomerList l          
	INNER JOIN dbo.tblArCust c
		ON l.CustId = c.CustId          
	WHERE
		c.AcctType = 1          
	AND
		(c.StmtInvcCode = 1 OR c.StmtInvcCode = 3)          
...
BEGIN          
  ...
  --adjust each bucket for over applied UnappliedCredit          
	...
	SET
		AmtCurrent =	CASE
							WHEN AmtCurrent > 0 THEN AmtCurrent
							ELSE 0 
						END,          
		AmtDue1 =	CASE
						WHEN AmtDue1 > 0 THEN AmtDue1
						ELSE 0
					END,          
		AmtDue2 =	CASE
						WHEN AmtDue2 > 0 THEN AmtDue2 
						ELSE 0 
					END,          
		AmtDue3 =	CASE 
						WHEN AmtDue3 > 0 THEN AmtDue3 
						ELSE 0 
					END,          
		AmtDue4 =	CASE 
						WHEN AmtDue4 > 0 THEN AmtDue4 
						ELSE 0 
					END,          
		UnpaidFinch =	CASE 
							WHEN UnpaidFinch > 0 THEN UnpaidFinch 
						ELSE 0 
						END,          
		UnappliedCredit =	CASE 
								WHEN UnappliedCredit < 0 THEN ABS(UnappliedCredit) 
								ELSE 0 
							END          
END          
...
INSERT INTO
	...
	INNER JOIN dbo.tblArCust c 
		ON l.CustId = c.CustId         
	LEFT OUTER JOIN dbo.ALP_tblArOpenInvoice alpO 
		on i.[Counter] = alpO.[AlpCounter]          
	LEFT OUTER JOIN dbo.ALP_tblArAlpSite alpS 
		on  alpO.AlpSiteID = alpS.SiteId        
	WHERE
		c.AcctType = 0 
	AND 
		i.TransDate <= @CutoffDate       
	--AND i.[Status] <> 1       
	--mah added @IncludeOnlyOpenInvoices condition:      
	AND
		(
			(
				@IncludeOnlyOpenInvoices = 1
				and i.[Status] <> 1 
				and i.[Status] <> 4
			)      
			OR
...
SET
	AlpSiteName =	CASE
						WHEN alpS.AlpFirstName is null THEN alpS.SiteName      
						WHEN alpS.AlpFirstName = '' THEN alpS.SiteName      
						WHEN alpS.AlpFirstName = ' ' THEN alpS.SiteName      
						ELSE alpS.SiteName + ', ' + alpS.AlpFirstName      
					END,       
	--alpS.SiteName,       
	AlpSiteAddr1 = alpS.Addr1, 
	AlpSiteAddr2 = alpS.Addr2      
FROM
	#AgeInvoiceListBySite BySite 
	INNER JOIN dbo.ALP_tblArAlpSite alpS 
		on  BySite.AlpSiteID = alpS.SiteId        

 --issue an aging of the identified invoices          
INSERT INTO
	#InvoiceAging          
EXEC
	dbo.trav_ArInvoiceAging_proc
		@StatementDate,
		@InvcFinch,
		1
...
INSERT INTO
	...
	SELECT 
		...
		AcctType, 
		0, 
		ISNULL(c.NewFinch, 0) + ISNULL(b.UnpaidFinch, 0), 
		ISNULL(b.AmtCurrent, 0), 
		ISNULL(b.AmtDue1, 0), 
		ISNULL(b.AmtDue2, 0), 
		ISNULL(b.AmtDue3, 0), 
		ISNULL(b.AmtDue4, 0), 
		ISNULL(b.UnappliedCredit, 0),      
	...
	LEFT JOIN
		(
			SELECT
				CustId, 
				SUM(UnappliedCredit) UnappliedCredit, 
				SUM(UnpaidFinch) UnpaidFinch, 
				SUM(AmtCurrent) AmtCurrent, 
				SUM(AmtDue1) AmtDue1, 
				SUM(AmtDue2) AmtDue2, 
				SUM(AmtDue3) AmtDue3, 
				SUM(AmtDue4) AmtDue4          
  ...
  --TRY grouping together payments that negate each other?  ( Negative Cash Receipt )?    
INSERT INTO
	#InvoiceActivity
		(
			[Counter], 
	...
	SELECT
		...
		--mah 04/01/15:    
		CASE
			WHEN (i.RecType = -2 AND LEFT(i.InvcNum ,6) <> 'ON ACC' AND LEFT(i.InvcNum ,9) <> 'LATE CHRG') THEN    
				CASE     
					WHEN i.PmtMethodId IS NULL THEN CheckNum    
					WHEN i.PmtMethodId = '' THEN CheckNum    
					ELSE
						CASE
							WHEN CheckNum IS NULL THEN PmtMethodId    
							ELSE PmtMethodId + ' ' + CheckNum
						END    
				END    
			ELSE    
				CASE
					WHEN i.AmtFgn > 0 THEN i.CustPONum     
					WHEN (LEFT(i.InvcNum ,6) <> 'ON ACC' AND LEFT(i.InvcNum ,9) <> 'LATE CHRG') THEN CheckNum    
					ELSE ''
				END    
		END AS CheckNum,          
			--, CASE WHEN i.Rectype = 1 OR i.Rectype = -1 THEN i.CustPONum ELSE CheckNum END AS CheckNum          
		CASE
			WHEN i.RecType > 0 THEN AmtFgn 
			ELSE 0
		END AS Charges,
		CASE
			WHEN i.RecType < 0 THEN AmtFgn 
			ELSE 0 
		END AS Credits,
		SIGN(i.RecType) * AmtFgn AS AmountDue,
		0,
		BySite.AlpSiteId,
		BySite.AlpSiteName,
		BySite.AlpSiteAddr1, 
	...
	---MAH 11/18/14:  
	----mah 5/5/15:- change to inner join to stop display of old purged invoice data      
	--LEFT OUTER JOIN   
	INNER JOIN #AgeInvoiceListBySite BySite 
		on i.CustId = BySite.CustId
		and i.InvcNum = BySite.InvcNum      
	LEFT OUTER JOIN dbo.ALP_tblArOpenInvoice alpO 
		on i.[Counter] = alpO.[AlpCounter]      
	--MAH 11/18/14:          
	...
	LEFT OUTER JOIN dbo.ALP_tblArAlpSite alpS
		on  BySite.AlpSiteID = alpS.SiteId          
	WHERE 
		((i.TransDate <= @CutoffDate AND c.AcctType = 0) OR c.AcctType = 1)           
	--mah 04/01/15:    
	--AND i.[Status] = 0     
	AND
		i.Rectype <> -3 --exclude gain/loss invoices          
	AND
		(c.StmtInvcCode = 1 OR c.StmtInvcCode = 3)
	AND
		i.RecType <> 5 --Exclude pro forma invoice       
UNION ALL          
	...
	WHERE
		...
		--AND i.[Status] = 0     
	AND
		i.Rectype = 5 --Only pro forma invoice          
	AND
		(c.StmtInvcCode = 1 OR c.StmtInvcCode = 3)          
--end mod         
...
SELECT
	@FiscalYear = GlYear, 
	@FiscalPeriod = GlPeriod          
FROM
	dbo.tblSmPeriodConversion           
WHERE 
	@CutoffDate BETWEEN BegDate AND EndDate          
          
--  capture the YTD finance charges from history          
UPDATE
	#CustomerSummary
SET
	YTDFinch = ISNULL(finch.YTDFinch, 0)          
FROM
	(
		SELECT
			f.CustId, 
			SUM(f.FinchAmtFgn) YTDFinch           
		FROM
			dbo.tblArHistFinch f          
		WHERE
			f.FiscalYear = @FiscalYear
		AND
			f.GlPeriod <= @FiscalPeriod          
		GROUP BY 
			f.CustId
	) finch          
WHERE
	#CustomerSummary.CustId = finch.CustId          


CREATE TABLE #InvoiceActivityGrouped (
	CustId pCustId,           
	InvcNum varchar(15),           
	GroupType tinyint NOT NULL,        
  InvoiceCount int DEFAULT(0),
	AmountDue pDec DEFAULT(0),           
	TransDate datetime,           
	INDEX IX_#InvoiceActivityGrouped NONCLUSTERED (CustId, InvcNum, GroupType)
);
INSERT #InvoiceActivityGrouped
  (CustId, InvcNum, GroupType, InvoiceCount, AmountDue, TransDate)
SELECT 
	a.CustId, 
	a.InvcNum, 
	a.GroupType, 
	COUNT(1) AS InvoiceCount, 
	SUM(a.AmountDue) AS AmountDue,     
	--mah 04/02/15:    
...
SELECT
	...
	CASE
		WHEN ac.AlpCommYn = 1 THEN c.CustName      
	ELSE      
		CASE
			WHEN ac.AlpFirstName is null THEN c.CustName       
			ELSE ac.AlpFirstName + ' ' + c.CustName
		END      
	END AS CustName,      
	--, c.CustName      
	c.Attn, 
	...
	c.Country, 
	c.TaxExemptId, 
	(c.CreditLimit - c.CurAmtDue) AS RemainingCredit, 
	c.CalcFinch, 
	s.YTDFinch, 
	c.NewFinch, 
	s.UnpaidFinch AS Finch, 
	...
	--mah ADT custom change 04/02/15 ( do NOT show Easy Pay comment )      
	--mah 06/13/14 - made terms description appear as 'Easy Pay' for Credit Card and ACH customers,        
	CASE
		WHEN s.GroupCode = 'A' THEN 'Easy Pay'      
	...
	CAST(
		CASE
			WHEN (s.AmtDue3 + s.AmtDue4) > 0 THEN 3            
			ELSE
				CASE
					WHEN s.AmtDue2 > 0 THEN 2          
					ELSE
						CASE
							WHEN s.AmtDue1 > 0 THEN 1          
							ELSE 0
						END          
				END          
		END AS tinyint
	...
	acomm.GCStmtComment, 
	CASE
		WHEN s.GroupCode = 'A' THEN 'DEBIT NOTICE'      
		WHEN s.GroupCode = 'C' THEN 'DEBIT NOTICE'      
		ELSE 'STATEMENT'
	END AS AlpDocHeading, 
...
FROM
	...
	INNER JOIN dbo.ALP_tblArCust ac 
		ON s.CustId = ac.AlpCustId         
	LEFT JOIN dbo.tblArTermsCode t 
		ON c.TermsCode = t.TermsCode        
	LEFT JOIN dbo.ALP_tblArAlpStmtComment acomm 
		ON c.GroupCode = acomm.GroupCode          
WHERE
	(@PrintStatements = 0)           
OR
	(@PrintStatements = 1 AND EXISTS(SELECT * FROM #InvoiceActivity a WHERE a.CustId = s.CustId))           
OR
	(@PrintStatements = 2 AND (s.AmtCurrent + s.AmtDue1 + s.AmtDue2 + s.AmtDue3 + s.AmtDue4 + s.UnpaidFinch - s.UnappliedCredit) <> 0)           
OR
	(@PrintStatements = 3 AND (s.AmtCurrent + s.AmtDue1 + s.AmtDue2 + s.AmtDue3 + s.AmtDue4 + s.UnpaidFinch - s.UnappliedCredit) > 0)           
OR
	(@PrintStatements = 4 AND (s.AmtDue1 + s.AmtDue2 + s.AmtDue3 + s.AmtDue4) > 0)           
OR
	(@PrintStatements = 5 AND (s.AmtDue2 + s.AmtDue3 + s.AmtDue4) > 0)           
OR 
	(@PrintStatements = 6 AND (s.AmtDue3 + s.AmtDue4) > 0)           
	--MAH       
ORDER BY
	c.PostalCode, 
	c.CustId      
...
SELECT 
	i.CustId, 
	i.TransDate AS InvoiceDate, 
	i.InvcNum AS InvoiceNo, 
	i.RecType, 
	i.CheckNum AS CheckNo, 
	i.Charges,      
	--, i.Credits      
	0 AS Credits,
	...
	CASE
		WHEN @InvoiceSort = 0 THEN i.InvcNum       
		WHEN @InvoiceSort = 1 THEN CONVERT(nvarchar, i.TransDate, 112)       
		ELSE CONVERT(varchar(8),i.AlpSiteId) 
	END AS GrpId, 
	t.InvoiceCount, 
	i.GroupType,          
	...
	i.AlpSiteAddr2,       
	CASE
		WHEN i.AlpSiteAddr1 is null	THEN i.AlpSiteAddr2       
		ELSE
			CASE
				WHEN i.AlpSiteAddr2 IS NULL THEN i.AlpSiteAddr1      
...
FROM
	#CustomerSummary s          
	INNER JOIN #InvoiceActivity i
		ON s.CustId = i.CustId       
    --mah 06/13/14 - added details of the transaction      
    INNER JOIN dbo.tblArHistHeader h 
		ON i.InvcNum = h.InvcNum      
		--mah 01/25/15:    
		AND i.TransID = h.TransId    
		AND  i.CustId = h.CustId      
	INNER JOIN dbo.tblArHistDetail d
		ON h.TransId = d.TransId          
	INNER JOIN #InvoiceActivityGrouped t          
		ON i.CustId = t.CustId
		AND i.InvcNum = t.InvcNum
		AND i.GroupType = t.GroupType          
...
--get the Payment details  (Non ON ACCT )    
SELECT
	i.CustId,     
	--mah 04/02/15:    
	i.TransDate AS InvoiceDate, 
	i.InvcNum AS InvoiceNo, 
	i.RecType,     
	--t.TransDate AS InvoiceDate, i.InvcNum AS InvoiceNo, i.RecType         
	--, CASE WHEN i.CheckNum IS NULL THEN '' ELSE 'Ck# ' + i.CheckNum END AS CheckNo     
	CASE 
		WHEN i.CheckNum IS NULL THEN ''     
		WHEN i.CheckNum <> '' THEN i.CheckNum     
		--WHEN i.CheckNum <> '' THEN 'Ck# ' + i.CheckNum     
		ELSE ''
	END AS CheckNo, 
	0 AS Charges, 
	(-1 * i.Credits) AS Credits, 
	0 - i.Credits AS Balance,          
	--, CASE WHEN @InvoiceSort = 0 THEN i.InvcNum ELSE CONVERT(nvarchar, i.TransDate, 112) END AS GrpId         
	CASE 
		WHEN @InvoiceSort = 0 THEN i.InvcNum       
		WHEN @InvoiceSort = 1 THEN CONVERT(nvarchar, i.TransDate, 112)       
	...
	--, '' AS AddnlDesc       
	CASE
		WHEN d.PartId IS NULL THEN ''  
		ELSE d.PartId 
	END,    
	CASE 
		WHEN d.[Desc] IS NULL THEN ''  
		ELSE d.[Desc] 
	END,   
	CASE 
		WHEN d.AddnlDesc IS NULL THEN '' 
		ELSE CONVERT(varchar(2000), d.AddnlDesc) 
	END as AddnlDesc,     
	'' AS ExtPrice,      
	h.TransId,   
	--mah 04/03/15:     
	--, '' AS EntryNum   
	CASE 
		WHEN i.RecType = -2 THEN i.Counter 
		ELSE '' 
	END AS EntryNum, 
	t.AmountDue AS AlpInvcBalance      
	--end of mah additions      
FROM
	...
	LEFT OUTER JOIN dbo.tblArHistHeader h 
		ON i.InvcNum = h.InvcNum      
		AND  i.CustId = h.CustId      
		--mah 01/25/15:    
		AND i.TransID = h.TransId    
	LEFT OUTER JOIN dbo.tblArHistDetail d 
		ON h.TransId = d.TransId       
	INNER JOIN #InvoiceActivityGrouped t          
		ON i.CustId = t.CustId 
		AND i.InvcNum = t.InvcNum 
...
WHERE 
	(i.RecType <= 0) 
AND
	LEFT (i.InvcNum,6 ) <> 'ON ACC' 
AND 
	LEFT(i.InvcNum ,9) <> 'LATE CHRG' 
AND 
	t.AmountDue <> 0      
AND 
	(
...
--mah 04/02/15: separate ON ACCT payments from other payments/invoices    

UNION      

--get the Payment details  - ON ACCT ONLY    
SELECT
	i.CustId,     
	--mah 04/02/15:    
	--i.TransDate AS InvoiceDate, i.InvcNum AS InvoiceNo, i.RecType     
	t.TransDate AS InvoiceDate, 
	i.InvcNum AS InvoiceNo, 
	i.RecType,         
	--mah 04/03/15:  
	--, '' AS CheckNo   
	...
	0 AS Charges,     
	--mah 04/04/15:  
	t.AmountDue AS Credits, 
	t.AmountDue AS Balance,    
	--,(-1 * i.Credits) AS Credits, t.AmountDue AS Balance    
...
--get the Payment details  - LATE CHRG ONLY    
SELECT distinct
	i.CustId,     
	--mah 04/02/15:    
	--i.TransDate AS InvoiceDate, i.InvcNum AS InvoiceNo, i.RecType     
...
--    SELECT '#AgeInvoiceListBySite', * FROM #AgeInvoiceListBySite ORDER BY InvcNum  
--    SELECT '#InvoiceActivity', * FROM #InvoiceActivity ORDER BY InvcNum  


--drop table #CustomerSummary
--drop table #InvoiceAging
--drop table #AgeInvoiceList
--drop table #AgeInvoiceListBySite
--drop table #InvoiceActivity
--drop table #customerlist
--drop table #InvoiceActivityGrouped