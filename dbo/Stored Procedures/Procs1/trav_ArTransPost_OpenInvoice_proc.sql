...
CREATE PROCEDURE dbo.trav_ArTransPost_OpenInvoice_proc
AS
Set NoCount ON
BEGIN TRY
--PET:http://webfront:801/view.php?id=236845
--MOD:Finance Charge Enhancements
--MOD:Deposit Invoices - Excludes ProForma Invoice 
	...
	INSERT dbo.tblArOpenInvoice (CustId, RecType, InvcNum, TransDate, DistCode, TermsCode, CurrencyId, ExchRate, Amt, AmtFgn
		...
		, CustPONum, SourceApp) 
	SELECT h.CustId, TransType
		, CASE WHEN h.TransType < 0 
			THEN ISNULL(NULLIF(h.OrgInvcNum,''), ISNULL(NULLIF(h.InvcNum,''), b.DefaultInvoiceNumber)) 
			ELSE ISNULL(NULLIF(h.InvcNum,''), b.DefaultInvoiceNumber)
			END
		, InvcDate, DistCode, TermsCode, CurrencyID, ExchRate
		, TaxSubtotal + NonTaxSubtotal + SalesTax + TaxAmtAdj + Freight + Misc
		, TaxSubtotalFgn + NonTaxSubtotalFgn + SalesTaxFgn  + TaxAmtAdjFgn + FreightFgn + MiscFgn
		...
		, (CASE WHEN TransType < 0 THEN ISNULL(NULLIF(h.InvcNum,''), b.DefaultInvoiceNumber) ELSE NULL END)
		, CustPONum, CASE WHEN h.ProjItem = 'PR' THEN 3 ELSE 0 END   
	...
	SELECT h.CustId, -3, ISNULL(NULLIF(h.OrgInvcNum,''), ISNULL(NULLIF(h.InvcNum,''), b.DefaultInvoiceNumber))
		, InvcDate, DistCode, TermsCode, @CurrBase, 1, h.CalcGainLoss, 0, 0
		, 0, NetDueDate, NULL, GlPeriod, FiscalYear, @PostRun, h.TransId, 1
		, CASE WHEN h.ProjItem = 'PR' THEN 3 ELSE 0 END 
	FROM dbo.tblArTransHeader h 
	INNER JOIN #PostTransList b on h.TransId = b.TransId
	...
	BEGIN
		...
		SELECT h.CustId, h.InvcNum, GainLossAmt, 0, 0, 0, -3, i.PmtMethodId, i.CheckNum, @CurrBase, 1
			, i.TransDate, i.NetDueDate, i.DistCode, i.GlPeriod, i.FiscalYear, @PostRun, i.TransId, 1, 0
		FROM #GainLossInvoice h INNER JOIN dbo.tblArOpenInvoice i ON h.CreditCounter = i.[Counter] AND i.RecType <> 5
		
		UPDATE dbo.tblArTransHeader SET CalcGainLoss = CalcGainLoss + g.GainLossAmt
		FROM dbo.tblArTransHeader INNER JOIN dbo.tblArOpenInvoice i 
				ON dbo.tblArTransHeader.CustId = i.CustId AND dbo.tblArTransHeader.TransId = i.TransId AND i.RecType <> 5
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_ArTransPost_OpenInvoice_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_ArTransPost_OpenInvoice_proc';
