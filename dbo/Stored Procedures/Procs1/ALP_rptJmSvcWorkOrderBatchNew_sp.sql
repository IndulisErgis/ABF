...
CREATE PROCEDURE dbo.ALP_rptJmSvcWorkOrderBatchNew_sp          
...
SELECT dbo.ALP_tblJmSvcTkt.TicketId, dbo.ALP_tblJmSvcTkt.ProjectId, dbo.ALP_tblJmSvcTkt.PrefTime,           
 CONVERT(varchar,ALP_tblJmSvcTkt.CreateDate,1) AS CreateDate, ALP_tblJmSvcTkt.CreateBy, dbo.ALP_tblJmSvcTkt.CustId,           
 ...
 + coalesce(ALP_tblArAlpSite.PostalCode,'') AS CityState, ALP_tblJmSvcTkt.Contact,           
 ALP_tblJmSvcTkt.ContactPhone, ALP_tblJmSvcTkt.SysId, ALP_tblArAlpSiteSys.SysDesc, ALP_tblArAlpSiteSys.CentralId,           
 ALP_tblArAlpSiteSys.AlarmId, CASE WHEN LeaseYn = 1 THEN 'Yes' Else 'No' END AS LseYn,           
 ALP_tblArAlpSiteSys.WarrExpires, ALP_tblArAlpRepairPlan.RepPlan, ALP_tblJmSvcTkt.PriceId,           
 ALP_tblArCust_view.AlpJmCustLevel, ALP_tblJmWorkCode.WorkCode, ALP_tblJmTech.[Name] AS TechName,           
 ...
 CASE WHEN Structure = 1 THEN 'Ranch' WHEN Structure = 2 THEN 'Two-Level'           
  WHEN Structure = 3 THEN 'Split-Level' WHEN Structure = 4 THEN 'Office'           
  WHEN Structure = 5 THEN 'Warehouse' Else '' END AS Struct,           
 CASE WHEN Basement = 1 THEN 'Full' WHEN Basement = 2 THEN 'Part'           
  WHEN Basement = 3 THEN 'Drop Ceiling' WHEN Basement = 4 THEN 'All Finished'           
  WHEN Basement = 5 THEN 'None' Else '' END AS Bsmt, CASE WHEN Attic = 0 THEN 'None'           
  WHEN Attic = 1 THEN 'Full' WHEN Attic = 2 THEN 'Part' WHEN Attic = 3 THEN 'Small' Else '' END AS Att,           
 ...
 CASE WHEN CreditHold = 1  THEN 'ON HOLD'    WHEN coalesce(ALP_tblArCust_view.CurAmtDue, 0)           
 + coalesce(ALP_tblArCust_view.BalAge1, 0) + coalesce(ALP_tblArCust_view.BalAge2, 0) + coalesce(ALP_tblArCust_view.BalAge3, 0)           
 + coalesce(ALP_tblArCust_view.BalAge4, 0) + (coalesce(ALP_tblArCust_view.UnapplCredit, 0) * -1)           
 - coalesce(ALP_tblArCust_view.UnpaidFinch, 0) > 0 AND           
 (ALP_tblArCust_view.BalAge1 > 0 Or ALP_tblArCust_view.BalAge2 > 0 Or ALP_tblArCust_view.BalAge3 > 0 Or ALP_tblArCust_view.BalAge4 > 0)           
 THEN 'Past Due' Else 'Current' END As CreditStatus      
...
WHERE           
      (dbo.ALP_tblJmSvcTkt.ProjectID IS NOT NULL AND dbo.ALP_tblJmSvcTkt.ProjectID <> '')          
      AND          
 ...
 OR          
      ...
      AND (@JobStatus is  null  or ALP_tblJmSvcTkt.Status=@JobStatus)                 
      AND (@WorkCode is  null or ALP_tblJmSvcTkt.WorkCodeId=@WorkCode)           
      and           
...
ORDER BY Case WHEN @Order = '1' THEN CONVERT(varchar,dbo.ALP_tblJmTimeCard.StartDate,1)          
 ...
 END ,
 ALP_tblJmTimeCard.StartTime --Added by NSK on 28 Dec 2016.