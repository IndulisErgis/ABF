...
CREATE Procedure [dbo].[ALP_lkpJmSiteSysLookUp]
...
IF (@ComboCTL <> 'S') AND (@CustID <> '--NONE--')        --(@CustID <> '') AND (@CustID IS NOT NULL)
   BEGIN 
	...
	SELECT ALP_tblArAlpSite.SiteId, ALP_tblArAlpSiteSys.SysId,
	...
	FROM  (ALP_tblArAlpSite 
		INNER JOIN #ALP_tblJm110CustSites
		 ...
		 INNER JOIN ALP_tblArAlpSiteSys ON ALP_tblArAlpSite.SiteId=ALP_tblArAlpSiteSys.SiteId	
	WHERE 	( #ALP_tblJm110CustSites.SiteID = ALP_tblArAlpSite.SiteId)  and ALP_tblArAlpSiteSys.SysId = @SysID
