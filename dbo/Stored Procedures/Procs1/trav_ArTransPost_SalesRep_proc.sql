...
CREATE PROCEDURE dbo.trav_ArTransPost_SalesRep_proc
AS
BEGIN TRY
	...
	INSERT INTO #tmpCommissions (TransID, SalesRep1Id, SalesRep2Id, InvcDate, TransType, SalesTax, TaxAmtAdj
		...
		SELECT h.TransID
			...
			, CASE WHEN TransType < 0 
				THEN -ROUND((QtyShipSell * isnull(UnitCommBasis, UnitCostSell)), @PrecCurr) 
		...
		INNER JOIN #PostTransList l on h.TransId = l.TransId
		WHERE PriceExt <> 0 OR (QtyShipSell * isnull(UnitCommBasis, UnitCostSell)) <> 0
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_ArTransPost_SalesRep_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_ArTransPost_SalesRep_proc';
