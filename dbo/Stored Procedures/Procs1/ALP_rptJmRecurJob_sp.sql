CREATE procedure [dbo].[ALP_rptJmRecurJob_sp]          
--Created for  EFI# 1454 08/25/04 MAH           
--modified 06/04/09 MAH:  added Active Price for the service, for display on the Recur JobEdit Report          
--modified 04/11/14 MAH:  added sort order          
--modified by NSK on 29 Jan 2015 for bug id 176      
(          
 @CustIdFrom pCustId,          
 @CustIdThru pCustId,          
 ...
 @NextDate datetime ,    
 @UserId varchar(50) -- added by NSK on 05 Oct 2015           
) 
...
INSERT INTO ALP_tmpJmRecurJob          
 ...
 SS.InstallDate, 
 --RBS.Status,--Commented by NSK on 08 Jul 2016
 --Below code added by NSK on 08 Jul 2016 to display status correctly. Bug id 400
 --start
 ...
 ,@UserId -- User ID added by NSK on 05 Oct 2015         
    FROM dbo.tblArCustClass CC RIGHT OUTER JOIN dbo.ALP_tblArAlpSiteRecJob RJ       
 ...
 --Below lines commented by NSK on Jan 28 2015 for bug id 176         
 --Start      
       
 --AND (S.SiteName Between @SiteNameFrom and @SiteNameThru)          
 --AND (B.Branch Between @BranchFrom AND @BranchThru)          
 --AND (W.WorkCode Between @WorkCodeFrom AND @WorkCodeThru)          
 --AND (C.CustId Between @CustIdFrom and @CustIdThru)          
 --AND (C.GroupCode Between @GroupFrom AND @GroupThru)          
 --AND (CC.ClassId Between @ClassFrom AND @ClassThru)          
