...
CREATE PROCEDURE dbo.trav_ArTransPost_Customer_proc
AS
SET NOCOUNT ON
BEGIN TRY
	...
	INNER JOIN (SELECT c.CustId
		, SUM(CASE WHEN h.TransType > 0 THEN h.TaxSubtotalFgn + h.NonTaxSubtotalFgn + h.SalesTaxFgn + h.TaxAmtAdjFgn + h.FreightFgn + h.MiscFgn ELSE 0 END) AS InvoiceTotal
		, SUM(CASE WHEN h.TransType > 0 THEN 0 ELSE h.TaxSubtotalFgn + h.NonTaxSubtotalFgn + h.SalesTaxFgn + h.TaxAmtAdjFgn + h.FreightFgn + h.MiscFgn END) AS CreditMemoTotal
	...
	INNER JOIN (SELECT h.CustId, h.InvcDate, ISNULL(h.InvcNum, b.DefaultInvoiceNumber) AS InvcNum
		, (h.TaxSubtotalFgn + h.NonTaxSubtotalFgn + h.FreightFgn + h.MiscFgn + h.SalesTaxFgn + h.TaxAmtAdjFgn) AS InvoiceAmount
		FROM dbo.tblArTransHeader h INNER JOIN #PostTransList b on h.TransId = b.TransId
		WHERE h.TransType > 0
			AND (h.TaxSubtotalFgn + h.NonTaxSubtotalFgn + h.FreightFgn + h.MiscFgn + h.SalesTaxFgn + h.TaxAmtAdjFgn) > 0
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_ArTransPost_Customer_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_ArTransPost_Customer_proc';
