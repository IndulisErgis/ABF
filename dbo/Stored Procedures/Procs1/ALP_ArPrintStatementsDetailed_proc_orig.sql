...
CREATE PROCEDURE [dbo].[ALP_ArPrintStatementsDetailed_proc_orig]   
@CutoffDate datetime,           
...
BEGIN TRY          
 ...
 INSERT INTO #AgeInvoiceList (CustId, InvcNum, RecType, AgingDate, AmountDue        
  ...
  --AND i.[Status] <> 1       
   ...
   AND ((@IncludeOnlyOpenInvoices = 1 and i.[Status] <> 1 and i.[Status] <> 4)      
  OR (@IncludeOnlyOpenInvoices <> 1 AND i.[Status] <> 1))          
   AND (c.StmtInvcCode = 1 OR c.StmtInvcCode = 3) AND i.RecType <> 5 --Exclude pro forma invoice           
   --mod end       
 ...
 SELECT i.[Counter], i.CustId, i.CurrencyId          
  , i.InvcNum, i.TransDate, i.RecType      
    --mah 04/01/15:    
  , CASE WHEN (i.RecType = -2 AND LEFT(i.InvcNum ,6) <> 'ON ACC') THEN    
  CASE     
   WHEN i.PmtMethodId IS NULL THEN CheckNum    
   WHEN i.PmtMethodId = '' THEN CheckNum    
   ELSE CASE WHEN CheckNum IS NULL THEN PmtMethodId    
     ELSE PmtMethodId + ' ' + CheckNum END    
  END    
 ELSE    
  CASE WHEN i.AmtFgn > 0 THEN i.CustPONum     
   WHEN LEFT(i.InvcNum ,6) <> 'ON ACC' THEN CheckNum    
   ELSE '' END    
 END AS CheckNum          
 ...
 FROM #CustomerList l          
   ...
   --LEFT OUTER JOIN   
   INNER JOIN   
   #AgeInvoiceListBySite BySite on i.CustId = BySite.CustId  and i.InvcNum = BySite.InvcNum      
   LEFT OUTER JOIN dbo.ALP_tblArOpenInvoice alpO on i.[Counter] = alpO.[AlpCounter]      
   --MAH 11/18/14:          
   --LEFT OUTER JOIN dbo.ALP_tblArAlpSite alpS on  alpO.AlpSiteID = alpS.SiteId        
 ...
 WHERE ((i.TransDate <= @CutoffDate AND c.AcctType = 0) OR c.AcctType = 1)           
  --mah 04/01/15:    
  --AND i.[Status] = 0     
  AND i.Rectype <> -3 --exclude gain/loss invoices          
  AND (c.StmtInvcCode = 1 OR c.StmtInvcCode = 3) AND i.RecType <> 5 --Exclude pro forma invoice       
 ...
 SELECT i.[Counter], i.CustId, i.CurrencyId          
  ...
  END    
 ELSE    
  CASE WHEN i.AmtFgn > 0 THEN i.CustPONum ELSE CheckNum END    
 END AS CheckNum            
 ...
 FROM #CustomerList l          
   ...
   --LEFT OUTER JOIN   
   INNER JOIN #AgeInvoiceListBySite BySite on i.CustId = BySite.CustId  and i.InvcNum = BySite.InvcNum       
   LEFT OUTER JOIN dbo.ALP_tblArOpenInvoice alpO on i.[Counter] = alpO.[AlpCounter]       
      --MAH 11/18/14:          
   --LEFT OUTER JOIN dbo.ALP_tblArAlpSite alpS on  alpO.AlpSiteID = alpS.SiteId        
   LEFT OUTER JOIN dbo.ALP_tblArAlpSite alpS on  BySite.AlpSiteID = alpS.SiteId             
 ...
 WHERE ((i.TransDate <= @CutoffDate AND c.AcctType = 0) OR c.AcctType = 1)           
  --mah 04/01/15    
  --AND i.[Status] = 0     
  AND i.Rectype = 5 --Only pro forma invoice          
  AND (c.StmtInvcCode = 1 OR c.StmtInvcCode = 3)          
 --end mod         
 ...
 SELECT i.CustId, i.TransDate AS InvoiceDate, i.InvcNum AS InvoiceNo, i.RecType          
 ...
 --mah 04/03/15:     
 --, '' AS EntryNum   
 , CASE WHEN i.RecType = -2 THEN  i.Counter ELSE '' END AS EntryNum     
 , t.AmountDue AS AlpInvcBalance      
  --end of mah additions      
 FROM #CustomerSummary s          
 INNER JOIN #InvoiceActivity i ON s.CustId = i.CustId       
 ...
 WHERE (i.RecType <= 0) AND LEFT (i.InvcNum,6 ) <> 'ON ACC' AND t.AmountDue <> 0      
 AND ((@PrintStatements = 0)            
  OR (@PrintStatements = 1 AND EXISTS(SELECT * FROM #InvoiceActivity a WHERE a.CustId = s.CustId))           
  OR (@PrintStatements = 2 AND (s.AmtCurrent + s.AmtDue1 + s.AmtDue2 + s.AmtDue3 + s.AmtDue4 + s.UnpaidFinch - s.UnappliedCredit) <> 0)           
  OR (@PrintStatements = 3 AND (s.AmtCurrent + s.AmtDue1 + s.AmtDue2 + s.AmtDue3 + s.AmtDue4 + s.UnpaidFinch - s.UnappliedCredit) > 0)           
 ...
 , '' AS CheckNo       
  , 0 AS Charges     
  --mah 04/04/15:  
  , t.AmountDue AS Credits, t.AmountDue AS Balance    
