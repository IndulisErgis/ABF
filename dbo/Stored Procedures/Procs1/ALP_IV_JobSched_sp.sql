...
CREATE PROCEDURE [dbo].[ALP_IV_JobSched_sp]             
...
BEGIN TRY          
SET @str = 'SELECT TicketId, dbo.ALP_ufxJmNextDate(TicketId) AS NxtDt INTO #ScheduledTickets FROM ALP_tblJmSvcTkt T WHERE T.Status = ''Scheduled''     
 SELECT  T.ProjectId,T.TicketId, T.Status, T.SiteId, T.CustId,        
 T.LeadTechId,        
...
FROM dbo.ALP_tblJmSvcTkt T          
 ...
 LEFT OUTER JOIN #ScheduledTickets ON  #ScheduledTickets.TicketId = T.TicketId '       
 + CASE WHEN @Where IS NULL THEN ' '              
  WHEN @Where = '' THEN ' '              
  WHEN @Where = ' ' THEN ' '              
  ELSE ' WHERE ' + @Where              
  END  + '         
  SELECT tmp.*, SiteName, S.Addr1, ISNULL(S.Addr2, '''') as Addr2, S.City, S.PostalCode as Zip, S.Block as Lot,        
   tech.Tech as LeadTech,     
   dbo.ALP_ufxJmLastDate(tmp.TicketId) as LastVisit,   
   dbo.ALP_ufxJmLastTech(tmp.TicketId) as LastTch,       
   Case When tmp.Status =''Scheduled'' then dbo.ALP_ufxJmNextTime(tmp.TicketId) else '''' End as NxtTm,        
   Case When tmp.Status =''Scheduled'' then dbo.ALP_ufxJmNextTech(tmp.TicketId) else '''' End as NxtTch,         
 ST.SysType, SS.SysDesc as SystemDesc,  SS.AlarmId,SB.Subdiv as SubDiv       
   FROM #temp  tmp       
   INNER JOIN dbo.ALP_tblJmWorkCode WC ON WC.WorkCodeId = tmp.WorkCodeId        
   INNER JOIN dbo.ALP_tblArAlpSiteSys SS ON SS.SysId = tmp.SysId        
   INNER JOIN dbo.ALP_tblArAlpSysType ST ON ST.SysTypeId = SS.SysTypeId        
   LEFT OUTER JOIN dbo.ALP_tblArAlpSite S ON S.SiteId = tmp.SiteId        
   LEFT OUTER JOIN dbo.ALP_tblJmResol RC  ON RC.ResolID = tmp.ResolId         
   LEFT OUTER JOIN dbo.ALP_tblJmTech tech ON tech.TechId = tmp.LeadTechId        
