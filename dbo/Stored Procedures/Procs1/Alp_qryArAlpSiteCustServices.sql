CREATE PROCEDURE dbo.Alp_qryArAlpSiteCustServices @Cust varchar(10), @Site int            
As      
--Below code modified by ravi on 09.18.2015,  
--Created new temp table script instead of create table from select query itself   
--The input @Site Param datatype has changed from varchar to int, the code modified by ravi on 09.21.2015     
SET NOCOUNT ON           
  CREATE TABLE #tempLastPriceRecord (RecBillServPriceId int null,RecBillServId int null,ServiceEndDate DateTime null )  
  INSERT INTO #tempLastPriceRecord(RecBillServPriceId,RecBillServId ,ServiceEndDate)  
SELECT    
   MAX([p].[RecBillServPriceId]) AS [RecBillServPriceId], [p].[RecBillServId],null --into #tempLastPriceRecord    
...
((recBill.SiteId )=@Site) AND ((recBill.CustId)=@Cust))      
  ...
  UPDATE #tempLastPriceRecord set ServiceEndDate =   CASE      
                    WHEN  b.EndBillDate   IS NULL THEN NULL     
                    ELSE   b.EndBillDate      
                   END    
  From   #tempLastPriceRecord a Inner Join  ALP_tblArAlpSiteRecBillServPrice b   
  on a.RecBillServPriceId =b.RecBillServPriceId     
