CREATE Procedure [dbo].[ALP_qryJmUpdateSvcTktStatus]
@ID int
AS
...
SET ALP_tblJmSvcTkt.Status = 'Scheduled', ALP_tblJmSvcTkt.RevisedDate = GetDate(), 
	ALP_tblJmSvcTkt.RevisedBy = @UserId
FROM ALP_tblJmSvcTkt 
	INNER JOIN ALP_tblJmTimeCard ON ALP_tblJmSvcTkt.TicketId = ALP_tblJmTimeCard.TicketId 
WHERE (ALP_tblJmSvcTkt.Status='New' 
	Or (ALP_tblJmSvcTkt.Status='Targeted' AND ALP_tblJmSvcTkt.PrefDate < ALP_tblJmTimeCard.StartDate ) )
AND ALP_tblJmSvcTkt.TicketId <> @ID
--Targeted
UPDATE ALP_tblJmSvcTkt 
SET ALP_tblJmSvcTkt.Status = 'Targeted', ALP_tblJmSvcTkt.RevisedDate = GetDate(), 
	ALP_tblJmSvcTkt.RevisedBy = @UserId, ALP_tblJmSvcTkt.ReschDate = Null
FROM ALP_tblJmSvcTkt LEFT JOIN ALP_tblJmTimeCard ON ALP_tblJmSvcTkt.TicketId = ALP_tblJmTimeCard.TicketId 
WHERE (((ALP_tblJmSvcTkt.Status)='Scheduled') 
	AND ((ALP_tblJmSvcTkt.PrefDate) Is Not Null) 
...
WHERE (((ALP_tblJmSvcTkt.Status)='Scheduled') 
	AND ((ALP_tblJmSvcTkt.PrefDate) Is Not Null) AND ( ALP_tblJmSvcTkt.PrefDate > ALP_tblJmTimeCard.StartDate ))
	AND ALP_tblJmSvcTkt.TicketId <> @ID
