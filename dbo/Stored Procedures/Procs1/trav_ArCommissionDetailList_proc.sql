...
CREATE PROCEDURE dbo.trav_ArCommissionDetailList_proc
...
BEGIN TRY
	...
	INSERT INTO #RepTotals (SalesRepId, RepAmtInvc, RepAmtPmt, RepAmtCogs, RepAmtPayOn, RepAmtLines) 
		SELECT c.SalesRepId, SUM(c.AmtInvc), SUM(c.AmtPmt), SUM(tmp.AmtCogs), SUM(tmp.RepAmtPayOn), SUM(tmp.AmtLines)
		FROM dbo.tblArCommInvc c 
			INNER JOIN (SELECT c2.SalesRepID, MIN(c2.[Counter]) AS [Counter], SUM(c2.AmtCogs) AS AmtCogs
							, SUM(AmtLines + AmtTax + AmtFreight + AmtMisc) AS RepAmtPayOn, SUM(AmtLines) AS AmtLines 
	...
	SELECT s.RunCode, s.[Name], c.*, c.AmtCogs AS AmtCogs1, c.CompletedDate AS CompletedDate
		...
		, CASE WHEN HoldYn <> 0 THEN 'Yes' ELSE 'No' END AS TxtHoldYn 
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_ArCommissionDetailList_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_ArCommissionDetailList_proc';
