CREATE PROCEDURE [dbo].[ALP_IV_R205c_SalesSummary_PV_041315TEST]   
...
SELECT     
 ...
 MIN(PI.FirstTicket) AS FirstTicket, 
 'NONE' AS Rep, '                      ' AS OtherRep 
INTO #temp    
...
--select 'After Rep Updates', * from #temp order by ProjectId desc

--SELECT #temp.ProjectId, Comm.SalesRep, SUM(Comm.CommAmt) from #temp
UPDATE #temp SET #temp.OtherRep = CASE WHEN #temp.OtherRep = '' THEN Comm.SalesRep --+ ' ' + SUM(Comm.CommAmt)
		ELSE CASE WHEN #temp.OtherRep = Comm.SalesRep THEN #temp.OtherRep
				ELSE #temp.OtherRep + ' ' +  Comm.SalesRep --+ ' ' + SUM(Comm.CommAmt)
				END
		END
from #temp
	INNER JOIN ALP_tblJmSvcTkt Tkt ON Tkt.ProjectId = #temp.ProjectId
	INNER JOIN ALP_tblJmSvcTktCommSplit Comm ON Comm.TicketId = Tkt.TicketId
--GROUP BY #temp.ProjectId,Comm.SalesRep
WHERE #temp.Rep <> Comm.SalesRep AND 
	Comm.CommAmt <> 0
