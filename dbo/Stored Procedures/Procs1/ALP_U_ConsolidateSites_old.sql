...
CREATE PROCEDURE [dbo].[ALP_U_ConsolidateSites_old]
(
...
BEGIN
	...
	SET @OldIdList = SUBSTRING(@OldIdList,@comma+1,@length )
					INSERT INTO #Results (Msg) SELECT ('ID removed, list is now = ' + @OldIdList)
...
BEGIN TRANSACTION 
	BEGIN TRY
			...
			INSERT INTO #Results (Msg) SELECT ('843a1 done;')
			...
			INSERT INTO #Results (Msg) SELECT ('843a done;')
			...
			INSERT INTO #Results (Msg) SELECT ('843b done;')
			...
			INSERT INTO #Results (Msg) SELECT ('843c done;')
			...
			INSERT INTO #Results (Msg) SELECT ('843d done;')
			...
			INSERT INTO #Results (Msg) SELECT ('843e done;')
			...
			INSERT INTO #Results (Msg) SELECT ('843f done;')
			...
			INSERT INTO #Results (Msg) SELECT ('843g done;')
			...
			INSERT INTO #Results (Msg) SELECT ('843h done;')
