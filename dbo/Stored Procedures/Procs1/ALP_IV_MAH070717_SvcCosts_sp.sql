...
CREATE PROCEDURE [dbo].[ALP_IV_MAH070717_SvcCosts_sp] 
 --Created 07/07/17 by MAH
(
...
SELECT Q009.TicketId,
	...
	CASE WHEN Q009.JobPrice IS NULL THEN 0.00 ELSE Round(Q009.JobPrice,2) END AS JobPrice,   
	CASE WHEN Q009.JobCost IS NULL THEN 0.00 ELSE Q009.JobCost END AS JobCost,   
...
FROM [dbo].[ufxALP_IV_ServiceContracts_JobInfo_MAH070117] ()  Q009
	LEFT OUTER JOIN ufxALP_IV_ServiceContracts_TC_MAH070117() Q007
		ON Q009.TicketId = Q007.TicketId
--WHERE ((Q009.ProjectId IS  NULL) OR   (Q009.ProjectId = '') OR (Q009.ProjectId = ' '))
--	AND ((Q009.Status <>  'Cancelled') OR (Q009.Status <> 'Canceled') )
WHERE ((Q009.Status <>  'Cancelled') OR (Q009.Status <> 'Canceled') )
	AND(Q009.WorkCode = 'SC-Inspect')
