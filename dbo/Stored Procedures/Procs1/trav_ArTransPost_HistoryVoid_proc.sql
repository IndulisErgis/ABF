...
CREATE PROCEDURE dbo.trav_ArTransPost_HistoryVoid_proc
AS
BEGIN TRY
	...
	INNER JOIN (SELECT h.CustId From dbo.tblArTransHeader h INNER JOIN #VoidTransList l ON h.TransId = l.TransId GROUP BY h.CustId) t on c.CustId = t.CustId
	...
	SELECT @PostRun, h.TransId, TransType, BatchId, CustId
		...
		, CASE WHEN h.TransType < 0 AND -h.CalcGainLoss <> 0 THEN --applies only to Credit Memos with an amount 
			...
			END  GlAcctGainLoss
		, h.VoidYn, ISNULL(pmt.TotPmtAmt, 0), ISNULL(pmt.TotPmtAmtFgn, 0), ISNULL(pmt.TotPmtGainLoss, 0), h.CF
	...
	FROM dbo.tblArTransHeader h 
	INNER JOIN dbo.tblArTransPmt d ON h.TransId = d.TransId 
	INNER JOIN #VoidTransList l ON h.TransId = l.TransId
...
END CATCH
GO
EXECUTE sp_addextendedproperty @name = N'Version', @value = '11.0.19137.3213', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_ArTransPost_HistoryVoid_proc';


GO
EXECUTE sp_addextendedproperty @name = N'Comment', @value = 'Build 19137', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'trav_ArTransPost_HistoryVoid_proc';
