...
CREATE PROCEDURE dbo.ALP_InSafetyStockAlertReport_proc  
...
BEGIN TRY  
 ...
 BEGIN  
  ...
  WHERE (l.QtySafetyStock > (CASE WHEN @CalculateQuantityOption = 0 THEN ISNULL(o.QtyOnHand,0) ELSE (ISNULL(o.QtyOnHand,0) - ISNULL(q.QtyCmtd,0)) END) OR l.QtySafetyStock = 0)   
   AND i.KittedYN = 0  
 END  
 ELSE  
 BEGIN  
  ...
  FROM #tmpItemLocationList t INNER JOIN tblInItemLoc l (NOLOCK) ON t.ItemId = l.ItemId AND t.LocId = l.LocId  
   ...
   LEFT JOIN (SELECT ud.ItemId, v.ConvFactor, ud.Uom, ud.DfltType FROM dbo.tblInItemUomDflt ud   
      INNER JOIN dbo.tblInItemUom v ON ud.ItemId = v.ItemId AND ud.Uom = v.Uom WHERE ud.DfltType = 1) AS ud2 ON i.itemId = ud2.ItemId  
  WHERE (l.QtySafetyStock > (CASE WHEN @CalculateQuantityOption = 0 THEN ISNULL(o.QtyOnHand,0) ELSE (ISNULL(o.QtyOnHand,0) - ISNULL(q.QtyCmtd,0)) END) AND l.QtySafetyStock > 0)   
   AND i.KittedYN = 0  
